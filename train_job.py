...
def main():
    ...
    # V13.4.1 deployment-path lock. Load the three training modules from
    ...
    def _load_exact_local_module(_name):
        ...
        if not _path.exists():
            raise RuntimeError(
                f"[V13.4.1-DEPLOY-PREFLIGHT] LOCAL_SOURCE_MISSING module={_name} path={_path}"
            )
        ...
        if _spec is None or _spec.loader is None:
            raise RuntimeError(
                f"[V13.4.1-DEPLOY-PREFLIGHT] LOCAL_IMPORT_SPEC_FAILED module={_name} path={_path}"
            )
    ...
    if (not _expected_build) or (_expected_build != _utils_build) or _missing_utils or _dashboard_tag != "dashboard-v13.4.1-stat-only-threshold-validation" or _utils_tag != "utils-v13.4.1-stat-only-threshold-validation" or _wrapper_tag != "wrapper-v13.4.1-stat-only-threshold-validation":
        raise RuntimeError(
            "[V13.4.1-DEPLOY-PREFLIGHT] MIXED_OR_STALE_DEPLOYMENT "
            ...
            "The running container itself contains a mixed source set; rebuild/redeploy the job image from one V13.4.1 bundle."
        )
    log_func(
        f"[V13.4.1-DEPLOY-PREFLIGHT] PASS build={_expected_build} "
