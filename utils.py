# V11.2: Expert Active/Direction/Intensity layer aligned with training
# --- Core Python ---
...
def update_power_ratings(
    ...
    *,
    ...
    Notes / fixes:
      ✅ ratings_history UPSERT on (Sport, Team, Method, Updated_At) (no dupes)
      ✅ ratings_current UPSERT on (Sport, Team, Method) (methods don't overwrite)
      ✅ Current Updated_At aligned to last processed Game_Start (not wall-clock), so deltas are sane
      ✅ SoS applied ONLY when sos_gamma > 0, and gated by min games to avoid early noise
      ✅ NCAAB stored as KenPom-style AdjEM (tempo-free efficiency margin per 100 possessions)
    """
    ...
    SPORT_CFG = {
        ...
        "NFL": dict(
            ...
            sos_half_life_days=90, sos_gamma=0.0,
            season_gap_days=120, season_rho=0.75,
            blowout_mov=17, blowout_var_mult=2.25,
        ),
        ...
        "NCAAF": dict(
            ...
            blowout_mov=21, blowout_var_mult=2.50,
        ),
        ...
        "WNBA": dict(
            ...
            blowout_mov=18, blowout_var_mult=2.00,
        ...
        "CFL": dict(
            ...
            blowout_mov=20, blowout_var_mult=2.10,
        ),
    ...
    # Optional: require N games before applying market anchoring
    MARKET_MIN_GAMES_BY_SPORT = {
        "NFL":  4,
        "NCAAF": 4,
        "NBA":  8,
        "WNBA": 6,
    ...
    def upsert_current(rows: list[dict]):
        ...
        WHEN NOT MATCHED THEN
          INSERT (Sport, Team, Rating, Method, Updated_At)
    ...
    # ---------------- seed + state helpers ----------------
    def load_seed_ratings(sport: str, asof_ts: dt.datetime | pd.Timestamp | None, *, method: str | None = None) -> dict[str, float]:
        """
        Seed by SAME method when possible.
        If method is None, falls back to "any" method like your original design.
        """
        sport_u = sport.upper()
        method_l = (method or "").lower().strip() or None

        if asof_ts is None:
            if method_l:
                cur = bq.query(
                    f"SELECT Team, Rating FROM `{table_current}` WHERE UPPER(Sport) = @sport AND LOWER(Method) = @method",
                    job_config=bigquery.QueryJobConfig(
                        query_parameters=[
                            bigquery.ScalarQueryParameter("sport", "STRING", sport_u),
                            bigquery.ScalarQueryParameter("method", "STRING", method_l),
                        ]
                    )
                ).to_dataframe()
            else:
                cur = bq.query(
                    ...
                    job_config=bigquery.QueryJobConfig(
                        query_parameters=[bigquery.ScalarQueryParameter("sport", "STRING", sport_u)]
                    )
                ).to_dataframe()
            ...
            if method_l:
                hist = bq.query(
                    ...
                    WITH ranked AS (
                      ...
                      FROM `{table_history}`
                      WHERE UPPER(Sport) = @sport AND LOWER(Method) = @method
                    )
        ...
        if method_l:
            df = bq.query(
                f"""
                WITH latest AS (
                  SELECT Team, Rating, Updated_At,
                         ROW_NUMBER() OVER (PARTITION BY Team ORDER BY Updated_At DESC) AS rn
                  FROM `{table_history}`
                  WHERE UPPER(Sport) = @sport AND LOWER(Method) = @method AND Updated_At <= @asof
                )
        ...
        return dict(zip(df.Team, df.Rating)) if not df.empty else {}
    ...
    def get_last_ts(sport: str, *, method: str | None = None):
        params = [bigquery.ScalarQueryParameter("sport", "STRING", sport.upper())]
        method_l = (method or "").lower().strip() or None
        where_method = ""
        if method_l:
            where_method = "AND LOWER(Method) = @method"
            params.append(bigquery.ScalarQueryParameter("method", "STRING", method_l))

        df = bq.query(
            f"SELECT MAX(Updated_At) AS last_ts FROM `{table_history}` WHERE UPPER(Sport) = @sport {where_method}",
    ...
    # ---------------- streaming games (spread-close attach, used by kalman engines) ----------------
    def load_games_stream(
        ...
        *,
        ...
        if use_market:
            ...
            latest_per_book AS (
              SELECT
                ...
                TIMESTAMP(m.Snapshot_Timestamp) AS Snapshot_TS
        ...
        for df in stream_query_dfs(bq, sql_to_run, params=params, page_rows=page_rows, select_cols=None):
            ...
            df = df.sort_values(["Game_Start", "Snapshot_TS"], kind="mergesort")
    ...
    def run_kalman_offdef(
        *,
        ...
        sos_min_games = int(cfg.get("sos_min_games", 999999))
        ...
        pts_outlier_thresh = float(cfg.get("pts_outlier_thresh", 145.0))
        pts_outlier_mult   = float(cfg.get("pts_outlier_mult", 2.0))

        smooth_mov_start = float(cfg.get("smooth_mov_start", 10.0))
        smooth_mov_slope = float(cfg.get("smooth_mov_slope", 0.03))
        ...
        # seed state (elo-style only; keep original behavior)
        seed = load_seed_ratings(sport, window_start, method="elo_kalman")
        ...
        BATCH_SZ = 50_000
        utc_now_for_sos_decay = pd.Timestamp.now(tz="UTC")
        last_game_ts = None
        ...
        for chunk in load_games_stream(
            ...
            chunk["Game_Start"] = pd.to_datetime(chunk["Game_Start"], utc=True, errors="coerce")
    ...
    def run_kp_kalman_ppp(
        *,
        ...
        SELECT s.home, s.away, s.hs, s.as_, s.gs, tc.total_close
        FROM scores s
    ...
    for sport in sports:
        ...
        # For NCAAB, track last_ts by kp_adj_em so we don't mix with legacy ridge_massey history
        if sport.upper() == "NCAAB":
            last_ts = get_last_ts(sport, method="kp_adj_em")
        else:
        ...
        if cfg["model"] == "poisson":
            ...
            for chunk in load_games_stream(
                ...
                for _, g in chunk.iterrows():
                    ts = pd.to_datetime(g.Game_Start, utc=True, errors="coerce")
                    if pd.isna(ts):
                        continue
                    last_game_ts_poisson = ts if last_game_ts_poisson is None else max(last_game_ts_poisson, ts)
            ...
            current_ts = last_game_ts_poisson if last_game_ts_poisson is not None else pd.Timestamp.now(tz="UTC")
...
def write_sharp_moves_to_master(df, table='sharp_data.sharp_moves_master'):
    ...
    # Optional deterministic-system/state columns.  Schema evolution is fail-open:
    # if permissions do not allow adding fields, legacy upload continues unchanged.
    _sys_optional = _pathi_bigal_optional_cols(df)
    _sys_supported = _ensure_optional_bq_columns(df, table, _sys_optional)
    ALLOWED_COLS += [c for c in _sys_supported if c not in ALLOWED_COLS]
...
def add_line_and_crossmarket_features(df):
    ...
    # Override legacy movement names with the corrected canonical semantics.
    return override_corrected_line_move_features(df)
...
def compute_ev_features_sharp_vs_rec(
    ...
    *,
    ...
    """
    Backend-aligned EV calculation:
    - pairs sharp sides at GAME grain (Merge_Key_Short when available)
    - de-vigs same market pair
    - prices spreads/totals side-aware
    - returns same row count
    """
    def _phi_local(z):
    ...
    # one best row per side per book
    sharp_rows = (
    ...
    truth["Truth_Fair_Prob_at_SharpLine"] = np.where(good, p_this_raw / s, np.nan)
    truth["Sharp_Line"] = pd.to_numeric(sharp_line_this, errors="coerce")

    if sigma_col not in dm.columns:
        dm[sigma_col] = np.nan

    sig_keep = [pair_key, "Market", "Outcome_Norm", sigma_col]
    if "Sport" in dm.columns:
        sig_keep.append("Sport")

    sig_map = dm[sig_keep].drop_duplicates([pair_key, "Market", "Outcome_Norm"])
    ...
    dm = dm.merge(
        truth[keys + ["Truth_Fair_Prob_at_SharpLine", "Truth_Margin_Mu", "Truth_Sigma"]],
        on=keys,
        how="left",
    )
    ...
    if mask.any():
        z = (mu_series[mask].to_numpy(dtype=float, copy=False) +
             line_rec[mask].to_numpy(dtype=float, copy=False)) / sig_series[mask].to_numpy(dtype=float, copy=False)
        dm.loc[mask, "Truth_Fair_Prob_at_RecLine"] = _phi_local(z)
    ...
    if mask.any():
        z = (line_rec[mask].to_numpy(dtype=float, copy=False) -
             mu_series[mask].to_numpy(dtype=float, copy=False)) / sig_series[mask].to_numpy(dtype=float, copy=False)
        dm.loc[mask, "Truth_Fair_Prob_at_RecLine"] = _phi_local(z)
    ...
    dm.loc[mask, "Truth_Fair_Prob_at_RecLine"] = base_sharp[mask].values
...
def compute_pr_points_slopes_from_scores(
    ...
    And ratings_history columns:
      Sport, Team, Rating, Updated_At
    """
    from google.cloud import bigquery
    ...
    WITH base AS (
      ...
      WHERE Market = 'spreads'
        ...
        AND feat_Game_Start IS NOT NULL
    ...
    team_r AS (
      ...
      JOIN `{ratings_table}` r
       ...
       AND TIMESTAMP(r.Updated_At) <= TIMESTAMP_SUB(
            b.Game_Start, INTERVAL CAST(@lag_hours AS INT64) HOUR
          )
      QUALIFY ROW_NUMBER() OVER (
    ...
    both_r AS (
      ...
      JOIN `{ratings_table}` r
       ...
       AND TIMESTAMP(r.Updated_At) <= TIMESTAMP_SUB(
            t.Game_Start, INTERVAL CAST(@lag_hours AS INT64) HOUR
          )
      QUALIFY ROW_NUMBER() OVER (
...
# ============================================================
# BACKEND FIXES (ratings_current path)
#   Goals:
#   1) Never silently mix methods/scales (fixes “100 vs 2” weirdness)
#   2) Always import bigquery (your current code uses bigquery but doesn’t import it)
#   3) Apply sport aliases to Sports AND teams
#   4) Attach Home/Away_Rating_Method and PR_Pts_Beta_Used
#   5) Convert ratings units -> points using learned betas ONLY when needed
#      (skip conversion for zero-centered methods: kp_adj_em, nba_bpi_adj_em)
# ============================================================

# methods that are already “points-ish” (or at least not Elo-ish 1500 scale)
ZERO_CENTERED_METHODS = {"kp_adj_em", "nba_bpi_adj_em"}
...
def _canonize_sport_series(s: pd.Series, sport_aliases: dict) -> pd.Series:
    canon: dict[str, str] = {}
    ...
    return s.astype(str).str.upper().str.strip().map(lambda x: canon.get(x, x))

def _aliases_for_sport(sport: str, sport_aliases: dict) -> list[str]:
    sport = (sport or "").upper().strip()
    al = (sport_aliases or {}).get(sport, [])
    if not isinstance(al, list):
        al = [al]
    return sorted({sport, *[str(x).upper() for x in al]})
...
def enrich_power_from_current_inplace(
    ...
    *,
    ...
    strict_method: bool = True,         # ✅ NEW: don’t fall back to “any method” if preferred missing
    log_func=print,
    **_ignored_kwargs,
) -> pd.DataFrame:
    """
    Backend “ratings_current” enricher.

    Key behavior changes:
      - Always imports bigquery
      - Sport aliases applied (so NBA/NCAAB etc don’t mismatch)
      - Team normalization consistent
      - If preferred method exists and returns 0 rows, we *raise* when strict_method=True
        (prevents mixed-scale or wrong-method fallbacks)
      - Emits Home_Rating_Method / Away_Rating_Method for auditing
    """
    if df is None or df.empty:
        return df
    if bq is None:
    ...
    from google.cloud import bigquery  # ✅ FIX: your code referenced bigquery without importing
    ...
    # normalize
    df[sport_col] = _canonize_sport_series(df[sport_col], sport_aliases)
    df[home_col]  = _norm_team_series(df[home_col])
    df[away_col]  = _norm_team_series(df[away_col])
    ...
    # method audit outputs
    if "Home_Rating_Method" not in df.columns:
        df["Home_Rating_Method"] = ""
    if "Away_Rating_Method" not in df.columns:
        df["Away_Rating_Method"] = ""

    # NBA tempo outputs (optional columns)
    ...
    if df_cur is None or df_cur.empty:
        log_func("⚠️ ratings_current returned 0 rows; using baselines.")
        # baseline per row depends on preferred method
        for sp in sports:
            mask = df[sport_col].eq(sp)
            pref = str(preferred_method.get(sp, "")).lower().strip()
            base_sp = np.float32(0.0) if (pref in ZERO_CENTERED_METHODS) else np.float32(baseline)
            df.loc[mask, out_home_col] = base_sp
            df.loc[mask, out_away_col] = base_sp
            df.loc[mask, out_diff_col] = np.float32(0.0)
            df.loc[mask, "Home_Rating_Method"] = pref
            df.loc[mask, "Away_Rating_Method"] = pref
        return df

    df_cur["Sport"] = df_cur["Sport"].astype(str).str.upper().str.strip()
    df_cur["Team"] = _norm_team_series(df_cur["Team"])
    df_cur["Method"] = df_cur["Method"].astype(str).str.lower().str.strip()
    ...
    def _latest_team_map(d: pd.DataFrame) -> pd.DataFrame:
        # latest row per team (stable)
        d = d.dropna(subset=["Team", "Rating"]).copy()
        if d.empty:
            return d
    ...
    def _build_map_for_sport_method(sp: str, method_name: str) -> pd.DataFrame:
        d = df_cur.loc[(df_cur["Sport"] == sp) & (df_cur["Method"] == method_name)].copy()
        return _latest_team_map(d)

    def _build_latest_any_method(sp: str) -> pd.DataFrame:
    ...
    for sp in sports:
        ...
        pref = str(pref).lower().strip() if pref is not None else ""

        # sport-specific baseline
        base_sp = np.float32(0.0) if (pref in ZERO_CENTERED_METHODS) else np.float32(baseline)

        if pref:
            d_map = _build_map_for_sport_method(sp, pref)
            if d_map.empty:
                msg = f"⚠️ ratings_current has 0 rows for sport={sp} method={pref}"
                if strict_method:
                    raise RuntimeError(msg + " (strict_method=True)")
                log_func(msg + " → falling back to latest-any-method")
                d_map = _build_latest_any_method(sp)
        else:
            # no preferred method given
            d_map = _build_latest_any_method(sp)

        # build series
        s_rating = d_map.set_index("Team")["Rating"].astype("float32") if not d_map.empty else pd.Series(dtype="float32")
        s_method = d_map.set_index("Team")["Method"].astype(str) if ("Method" in d_map.columns and not d_map.empty) else pd.Series(dtype=str)

        hk = df.loc[mask, home_col]
        ak = df.loc[mask, away_col]

        h = s_rating.reindex(hk).to_numpy(dtype="float32", copy=False)
        a = s_rating.reindex(ak).to_numpy(dtype="float32", copy=False)

        hm = s_method.reindex(hk).astype(str).to_numpy(copy=False) if not s_method.empty else np.array([pref]*len(hk), dtype=object)
        am = s_method.reindex(ak).astype(str).to_numpy(copy=False) if not s_method.empty else np.array([pref]*len(ak), dtype=object)
        ...
        # audit methods
        df.loc[mask, "Home_Rating_Method"] = pd.Series(hm, index=df.index[mask]).astype(str).str.lower().values
        df.loc[mask, "Away_Rating_Method"] = pd.Series(am, index=df.index[mask]).astype(str).str.lower().values

        # NBA tempo (optional) — keep separate from rating scale
        if sp == "NBA":
            d_t = _build_map_for_sport_method("NBA", "nba_bpi_adj_t")
            if not d_t.empty:
                t_map = d_t.set_index("Team")["Rating"].astype("float32")
                ht = t_map.reindex(hk).to_numpy(dtype="float32", copy=False)
                at = t_map.reindex(ak).to_numpy(dtype="float32", copy=False)
                df.loc[mask, "Home_BPI_Tempo"] = np.where(np.isfinite(ht), ht, np.float32(100.0)).astype("float32")
                df.loc[mask, "Away_BPI_Tempo"] = np.where(np.isfinite(at), at, np.float32(100.0)).astype("float32")
            else:
                df.loc[mask, "Home_BPI_Tempo"] = np.float32(100.0)
                df.loc[mask, "Away_BPI_Tempo"] = np.float32(100.0)
...
def enrich_and_grade_for_training(
    ...
    *,
    ...
    """
    Backend spread enrichment:
      - pulls ratings_current (method-safe)
      - (optional) rescales Elo-ish ratings to points using learned betas
      - computes favorite-centric edges and maps to each outcome row
    """
    ...
    # normalize sport + team keys for stability before joins
    df0 = df_spread_rows.copy()
    df0["Sport"] = _canonize_sport_series(df0["Sport"], sport_aliases)
    df0["Home_Team_Norm"] = _norm_team_series(df0["Home_Team_Norm"])
    df0["Away_Team_Norm"] = _norm_team_series(df0["Away_Team_Norm"])

    df_games = df0[base_cols].drop_duplicates().reset_index(drop=True).copy()

    # 1) attach current ratings (method-safe)
    enrich_power_from_current_inplace(
        ...
        strict_method=True,      # ✅ prevents method mixing / scale weirdness
        log_func=log_func,
    )

    # 2) Convert ratings units -> points using learned betas
    #    Only for Elo-ish methods. For kp_adj_em / nba_bpi_adj_em we force beta=1.0
    beta_map = resolve_pr_beta_map(
        bq,
        project=project,
        dataset=dataset,
    ...
    # ✅ fallback betas are "points per 1 rating point"
    beta_fallback_map = {
        "NFL":   25.0,
        "NCAAF": 25.0,
        "NBA":   35.0,
    ...
    sp = df_games["Sport"].astype(str).str.upper().values
    ...
    # method-aware gate: zero-centered methods => beta=1
    m = df_games.get("Home_Rating_Method", "").astype(str).str.lower().values
    beta = np.where(np.isin(m, list(ZERO_CENTERED_METHODS)), 1.0, beta).astype("float32")
    df_games["PR_Pts_Beta_Used"] = beta

    # keep unit versions for debugging/audit
    for c in ["Home_Power_Rating", "Away_Power_Rating", "Power_Rating_Diff"]:
        if c in df_games.columns and f"{c}_Units" not in df_games.columns:
            df_games[f"{c}_Units"] = pd.to_numeric(df_games[c], errors="coerce").astype("float32")

    # apply scaling
    df_games["Home_Power_Rating"] = (pd.to_numeric(df_games["Home_Power_Rating"], errors="coerce").astype("float32") * beta).astype("float32")
    df_games["Away_Power_Rating"] = (pd.to_numeric(df_games["Away_Power_Rating"], errors="coerce").astype("float32") * beta).astype("float32")
    df_games["Power_Rating_Diff"] = (df_games["Home_Power_Rating"] - df_games["Away_Power_Rating"]).astype("float32")

    # 3) Consensus market
    g_cons = prep_consensus_market_spread_lowmem(
        df0, value_col=value_col, outcome_col=outcome_col
    )
    ...
    for c in ["PR_Pts_Beta_Used", "Home_Rating_Method", "Away_Rating_Method",
        ...
        if c in g_full.columns and c not in g_fc.columns:
            g_fc[c] = g_full[c]

    out = df0.merge(g_fc, on=game_key, how="left")

    # 5) Map game-level numbers to each outcome row
...
def predict_multihead_meta(bundle: dict, df_rows: pd.DataFrame, p_outcome, eps: float = 1e-6):
    """
    Use the saved situation/value/meta heads when a new multi-head artifact provides them.
    Returns None for legacy artifacts or on any incomplete/unsupported meta configuration,
    allowing the caller to preserve the legacy outcome probability path.
    """
    if not isinstance(bundle, dict) or df_rows is None or df_rows.empty:
        return None

    meta_model = bundle.get("meta_model")
    if meta_model is None or not hasattr(meta_model, "predict_proba"):
        return None
    ...
    def _matrix(cols, overlay_trust_map=None):
        ...
        if missing:
            ...
            if sys_missing:
                logger.warning("⚠️ Multi-head model requested missing system features: %s", sys_missing[:12])
        for c in cols:
    ...
    p_value = np.full(n, 0.5, dtype=np.float64)
    ...
    if model_situation is not None and X_situation is not None and X_situation.shape[1] > 0:
        try:
            ...
            logger.warning("⚠️ Situation-head live prediction failed; using neutral 0.5: %s", e)
    ...
    if model_value_cls is not None and X_value is not None and X_value.shape[1] > 0:
        try:
            ...
            logger.warning("⚠️ Value-classifier live prediction failed; using neutral 0.5: %s", e)
    if model_value_reg is not None and X_value is not None and X_value.shape[1] > 0:
        try:
    ...
    meta_df = pd.DataFrame({
        ...
        "Meta_P_Situation": np.clip(p_situation, eps, 1.0 - eps),
        "Meta_P_Value": np.clip(p_value, eps, 1.0 - eps),
        "Meta_Value_Reg": value_reg,
    }, index=df_rows.index)

    meta_features = [str(c) for c in (cfg.get("meta_features") or [])]
    if "Meta_Odds_Price" in meta_features:
        meta_df["Meta_Odds_Price"] = pd.to_numeric(df_rows.get("Odds_Price"), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
    if "Meta_Line_Value" in meta_features:
        meta_df["Meta_Line_Value"] = pd.to_numeric(df_rows.get("Value"), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)

    # New artifacts persist exact meta feature order. For transitional artifacts,
    # use the same fixed order as training and trim only if n_features_in_ demands it.
    if not meta_features:
        meta_features = ["Meta_P_Outcome", "Meta_P_Situation", "Meta_P_Value", "Meta_Value_Reg"]
        nfi = getattr(meta_model, "n_features_in_", None)
        if nfi is not None:
            if int(nfi) >= 5:
                meta_df["Meta_Odds_Price"] = pd.to_numeric(df_rows.get("Odds_Price"), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
                meta_features.append("Meta_Odds_Price")
            if int(nfi) >= 6:
                meta_df["Meta_Line_Value"] = pd.to_numeric(df_rows.get("Value"), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
                meta_features.append("Meta_Line_Value")
            meta_features = meta_features[:int(nfi)]

    for c in meta_features:
        if c not in meta_df.columns:
            meta_df[c] = 0.0
    X_meta = meta_df.reindex(columns=meta_features).replace([np.inf, -np.inf], np.nan).fillna(0.0).to_numpy(dtype=np.float32)

    try:
        raw = np.asarray(meta_model.predict_proba(X_meta)[:, 1], dtype=np.float64).reshape(-1)
    except Exception as e:
        logger.warning("⚠️ Meta-head live prediction failed; falling back to outcome head: %s", e)
        return None

    if len(raw) != n:
        return None

    # Training stores meta_calibrator as (kind, object): iso/platt/beta.
    cal_bundle = bundle.get("meta_calibrator")
    if isinstance(cal_bundle, (tuple, list)) and len(cal_bundle) >= 2:
        ...
        try:
            ...
            if kind == "identity":
                ...
                if hasattr(cal, "transform"):
                    ...
                    raw = cal.predict(x)
            elif kind == "platt" and hasattr(cal, "predict_proba"):
                raw = cal.predict_proba(x.reshape(-1, 1))[:, 1]
            elif kind == "beta":
                if hasattr(cal, "predict"):
                    raw = cal.predict(x.reshape(-1, 1))
...
def _resolve_feature_cols_like_training(bundle, model, df_canon, prefer_saved_only: bool = True):
    ...
    if isinstance(bundle, dict):
        # primary location; new multi-head artifacts may persist outcome columns separately
        fc = bundle.get("feature_cols_outcome") or bundle.get("feature_cols")
...
def attach_fair_value_bet_pass_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    V13.2.28 uses a threshold learned on chronological OOF bets and independently
...
def apply_blended_sharp_score(
    ...
    *,
    ...
    try:
        ...
        # Canonical market-aware direction fields (same semantics as training).
    ...
    for mkt in markets_present:
        ...
        try:
            # ===== MODEL SCORING =====
            # V11.5.9: reconstruct the historical/core prior from the fitted artifact
            # before resolving the saved Outcome feature matrix. Legacy bundles simply
            # receive neutral inactive columns.
            df_canon = apply_historical_core_runtime_feature(df_canon, bundle, mkt)
            df_canon = apply_ncaaf_statistical_brain_feature(df_canon, bundle.get("ncaaf_statistical_brain") if isinstance(bundle, dict) else None, mkt)
            # V13.0.16 specialists may use distilled Brain_* market/Pathi/BigAl
            # predictors. Build the exact training brain contract BEFORE V13 scoring.
            _brain_rows_before = len(df_canon)
            ...
            if str(sport).upper().strip() == "NCAAF":
                ...
                logger.info("[NCAAF-STAT-RUNTIME] market=%s active=%d/%d mean_trust=%.3f", mkt, _ns_active, len(df_canon), _ns_trust)
            # Brain_* predictors were attached above before V13 so the specialist
            # and legacy paths consume the same leakage-safe state.
            ...
            if not feature_cols or X_can.shape[1] == 0 or X_can.empty:
                ...
                outcome_preds = predict_blended(bundle, X_can, model=model, iso=iso)
                if outcome_preds is None or len(outcome_preds) != len(df_canon):
                    ...
                    preds = outcome_preds if meta_preds is None else meta_preds
                    if meta_preds is not None:
                        logger.info("🧠 %s: using saved three-head + meta inference", mkt.upper())
            ...
            if str(sport).upper().strip() == 'NCAAF' and str(mkt).lower().strip() == 'spreads':
                ...
                cols_to_write += [c for c in V13_SPECIALIST_OUTPUT_COLS if c in df_canon.columns and c not in cols_to_write]
        ...
        if df_inverse.empty or df_canon.empty:
            ...
            for _c in _canon_pred_cols:
                if _c.startswith('V13_'):
                    _rename[_c] = _c + '_opponent'
            df_canon_preds = (
                df_canon[_canon_pred_cols]
            ...
            if str(sport).upper().strip() == 'NCAAF' and str(mkt).lower().strip() == 'spreads' and 'V13_Fair_Margin_opponent' in df_inverse.columns:
                ...
                if 'V13_Own_Fair_Margin_opponent' in df_inverse.columns:
                    df_inverse['V13_Own_Fair_Margin']=-pd.to_numeric(df_inverse.get('V13_Own_Fair_Margin_opponent'),errors='coerce')
                if 'V13_Own_Cover_Prob_opponent' in df_inverse.columns:
                    df_inverse['V13_Own_Cover_Prob']=1.0-pd.to_numeric(df_inverse.get('V13_Own_Cover_Prob_opponent'),errors='coerce')
                if 'V13_Own_Push_Prob_opponent' in df_inverse.columns:
                    df_inverse['V13_Own_Push_Prob']=pd.to_numeric(df_inverse.get('V13_Own_Push_Prob_opponent'),errors='coerce')
                if 'V13_Market_Margin_opponent' in df_inverse.columns:
                    df_inverse['V13_Market_Margin']=-pd.to_numeric(df_inverse.get('V13_Market_Margin_opponent'),errors='coerce')
                if 'V13_Market_NoVig_Prob_opponent' in df_inverse.columns:
                    df_inverse['V13_Market_NoVig_Prob']=1.0-pd.to_numeric(df_inverse.get('V13_Market_NoVig_Prob_opponent'),errors='coerce')
                if 'V13_MarketResidual_Prob_opponent' in df_inverse.columns:
                    df_inverse['V13_MarketResidual_Prob']=1.0-pd.to_numeric(df_inverse.get('V13_MarketResidual_Prob_opponent'),errors='coerce')
                if 'V13_Final_Fair_Prob_opponent' in df_inverse.columns:
                    df_inverse['V13_Final_Fair_Prob']=1.0-pd.to_numeric(df_inverse.get('V13_Final_Fair_Prob_opponent'),errors='coerce')
                _ibe = _ncaaf_v13_break_even_prob(pd.to_numeric(df_inverse.get('Odds_Price'), errors='coerce'))
                df_inverse['V13_Market_BreakEven_Prob']=_ibe
                if 'V13_Own_Fair_Margin' in df_inverse.columns:
                    df_inverse['V13_Point_Edge']=pd.to_numeric(df_inverse['V13_Own_Fair_Margin'],errors='coerce')-pd.to_numeric(df_inverse.get('V13_Market_Margin',-_iv),errors='coerce')
                if 'V13_Own_Cover_Prob' in df_inverse.columns:
                    _iown=pd.to_numeric(df_inverse['V13_Own_Cover_Prob'],errors='coerce')
                    df_inverse['V13_Raw_Probability_Edge']=_iown-_ibe
                    df_inverse['V13_Own_BreakEven_Edge']=_iown-_ibe
                    _invig=pd.to_numeric(df_inverse.get('V13_Market_NoVig_Prob'),errors='coerce')
                    df_inverse['V13_Market_Fair_Probability_Edge']=_iown-_invig
                _iprof = _ncaaf_v13_profit_per_dollar(pd.to_numeric(df_inverse.get('Odds_Price'), errors='coerce'))
                _ip = pd.to_numeric(df_inverse['V13_Cover_Prob'], errors='coerce').to_numpy(dtype=float, na_value=np.nan)
                ...
                df_inverse['V13_Final_BreakEven_Edge']=_ip-_ibe
                df_inverse['V13_Final_EV_Per_Dollar']=df_inverse['V13_EV_Per_Dollar']
                for _c in ['V13_Active','V13_Horizon_Hours','V13_State_Freshness','V13_Current_Season_Games','V13_Result_Disagreement_Bucket','V13_Result_Profile_N','V13_Result_Historical_Closer_Rate','V13_Result_Historical_Direction_Accuracy','V13_Result_Historical_Avg_Error_Improvement','V13_Result_Historical_Closer_CI_Low','V13_Result_Historical_Closer_CI_High','V13_Result_Profile_Used_In_EV','V13_Status','V13_Version','V13_Promotion_Mode','V13_Early_Season_Warning','V13_Drift_Scale','V13_Maturity_Bucket','V13_Maturity_Beta','V13_Early_Situational_Active','V13_Fundamental_Overlay_Regime','V13_Market_Overlay_Regime','V13_Pathi_Overlay_Regime','V13_BigAl_Overlay_Regime','V12_Legacy_Model_Prob']:
                    _opp = _c + '_opponent'
                ...
                for _fam in ('Market','Pathi','BigAl'):
                    for _suffix in ('Overlay_Weight','Overlay_Active'):
                        ...
                        if _opp in df_inverse.columns:
                            df_inverse[_c]=df_inverse[_opp]
                    _c=f'V13_{_fam}_Overlay_Contribution'; _opp=_c+'_opponent'
                    if _opp in df_inverse.columns:
            ...
            if str(sport).upper().strip() == 'NCAAF' and str(mkt).lower().strip() == 'spreads' and df_inverse.get('V13_Promotion_Mode',pd.Series('',index=df_inverse.index)).astype(str).eq('ACTIVE_VALIDATED_ARTIFACT').any():
                df_inverse['Scoring_Market'] = 'spreads_v13_promoted'
            else:
            ...
            try:
                ...
                df_inverse = override_corrected_line_move_features(df_inverse)
...
def build_pathi_bigal_team_game_state(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    games["Season"] = _sys_num_series(games, "Season", default=np.nan).replace([np.inf, -np.inf], np.nan)
...
def add_pathi_bigal_rule_flags(state: pd.DataFrame) -> pd.DataFrame:
    ...
    if side_flags:
        ...
        _ratio=np.full(len(denom),np.nan,dtype=float)
        np.divide(own.to_numpy(dtype=float),denom,out=_ratio,where=np.asarray(denom)>0)
        s["System_Consensus_Ratio"]=_ratio
    else:
...
def add_ai_betting_brain_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def num(*names, default=np.nan):
        ...
        for name in names:
            if name in out.columns:
                s = s.where(s.notna(), pd.to_numeric(out[name], errors='coerce'))
    ...
    # ------------------------------------------------------------------
    # 11) NCAAF STATISTICAL expert state (V12).
    # Structural efficiency model is independently shadow-gated upstream.
    # ------------------------------------------------------------------
    stat_prob = num('NCAAF_Stat_Prob')
    stat_base = num('NCAAF_Stat_Market_Baseline_Prob', default=0.5).fillna(0.5)
    stat_edge = num('NCAAF_Stat_Edge')
    stat_edge = stat_edge.where(stat_edge.notna(), stat_prob - stat_base)
    stat_trust = num('NCAAF_Stat_Trust', default=0).fillna(0).clip(0,1)
    stat_active = num('NCAAF_Stat_Active', default=0).fillna(0).eq(1) & stat_edge.notna() & stat_edge.abs().ge(0.0025) & stat_trust.gt(0)
    stat_direction = sgn(stat_edge, eps=0.0025)
    stat_intensity = (np.tanh(stat_edge.abs().fillna(0)*10.0)*stat_trust).clip(0,1)
    install_expert('NCAAFStat', stat_active, stat_direction, stat_intensity)
    out['Brain_Expert_NCAAFStat_Prob'] = stat_prob.astype('float32')
    out['Brain_Expert_NCAAFStat_Trust'] = stat_trust.astype('float32')
    out['Brain_Expert_NCAAFStat_Edge'] = stat_edge.astype('float32')
...
def add_pathi_football_key_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _num_candidates(*names):
        ...
        for name in names:
            if name in out.columns:
                _cand = pd.to_numeric(out[name], errors="coerce")
                s = s.where(s.notna(), _cand)
        return s
...
# ---------------------------------------------------------------------------
# Backend adapters: preserve existing row keys/semantics and add system state.
# ---------------------------------------------------------------------------
...
_PATHI_BIGAL_HISTORY_DESIRED_COLS = [
    ...
    "Value", "Odds_Price", "First_Odds", "First_Line_Value", "Open_Odds", "Open_Value",
    ...
    "SHARP_HIT_BOOL", "ATS_Cover_Margin", "Score_Home_Score", "Score_Away_Score",
...
_PATHI_BIGAL_STATE_PERSIST_COLS = [
    ...
    "H2H_Fair_Prob", "Opening_H2H_Fair_Prob", "Spread_vs_H2H_Role_Aligned",
]


def _pathi_bigal_optional_cols(df: pd.DataFrame) -> list[str]:
    if df is None or df.empty:
        return []
    out = []
    for c in df.columns:
        sc = str(c)
        if sc.startswith(("Pathi_", "BigAl_", "System_")) or sc in _PATHI_BIGAL_STATE_PERSIST_COLS:
            out.append(sc)
    return list(dict.fromkeys(out))


def _canon_system_sport(sport: str | None) -> str:
    s = str(sport or "").strip().upper()
    if not s:
        return ""
    # Canonical key may arrive as API sport key or friendly alias.
    try:
        return normalize_sport(s)
    except Exception:
        pass
    for canon, aliases in (SPORT_ALIASES or {}).items():
        all_aliases = {str(canon).upper(), *[str(a).upper() for a in (aliases or [])]}
        if s in all_aliases:
            return str(canon).upper()
    return s


def _system_sport_aliases(canon: str) -> list[str]:
    canon = _canon_system_sport(canon)
    vals = {canon}
    for a in SPORT_ALIASES.get(canon, []) if 'SPORT_ALIASES' in globals() else []:
        vals.add(str(a).upper())
    if canon in SPORTS:
        vals.add(str(SPORTS[canon]).upper())
    return sorted(v for v in vals if v)


def _ensure_merge_key_short_for_systems(df: pd.DataFrame) -> pd.DataFrame:
    """Add Merge_Key_Short without altering the backend's line/outcome Game_Key."""
    out = df.copy()
    ...
    if "Commence_Hour" not in out.columns:
        ...
        out["Commence_Hour"] = pd.to_datetime(out["Commence_Hour"], errors="coerce", utc=True).dt.floor("h")

    if "Merge_Key_Short" not in out.columns:
        out["Merge_Key_Short"] = pd.NA
    mks = out["Merge_Key_Short"].astype("string")
    need = mks.isna() | mks.str.strip().isin(["", "nan", "none", "<na>"])
    if need.any() and {"Home_Team_Norm", "Away_Team_Norm"}.issubset(out.columns):
        h = out.loc[need, "Home_Team_Norm"].astype(str).str.lower().str.strip()
        a = out.loc[need, "Away_Team_Norm"].astype(str).str.lower().str.strip()
        ts = out.loc[need, "Commence_Hour"]
        out.loc[need, "Merge_Key_Short"] = [
            build_merge_key(hh, aa, tt) if pd.notna(tt) else None
            for hh, aa, tt in zip(h, a, ts)
        ]
    out["Merge_Key_Short"] = out["Merge_Key_Short"].astype("string").str.lower().str.strip()
    return out


def _physicalize_system_game_key(df: pd.DataFrame) -> pd.DataFrame:
    """Temporary system frame: Game_Key becomes the physical-game Merge_Key_Short."""
    out = _ensure_merge_key_short_for_systems(df)
    if "Game_Key" in out.columns:
        out["__Backend_Line_Game_Key"] = out["Game_Key"].astype("string")
    else:
        out["__Backend_Line_Game_Key"] = pd.Series(pd.NA, index=out.index, dtype="string")
    out["Game_Key"] = out["Merge_Key_Short"].astype("string").str.lower().str.strip()
    return out
...
def _query_pathi_bigal_history_table(table_fq: str, sport: str, days_back: int | None) -> pd.DataFrame:
    ...
    if not cols_present:
        return pd.DataFrame()
    cols = [c for c in _PATHI_BIGAL_HISTORY_DESIRED_COLS if c in cols_present]
    required = {"Sport", "Market", "Outcome"}
    if not required.issubset(cols_present):
        return pd.DataFrame()
...
def _attach_final_scores_to_system_history(hist: pd.DataFrame, sport: str, days_back: int | None) -> pd.DataFrame:
    ...
    if {"Score_Home_Score", "Score_Away_Score"}.issubset(h.columns):
        if pd.to_numeric(h["Score_Home_Score"], errors="coerce").notna().any() and pd.to_numeric(h["Score_Away_Score"], errors="coerce").notna().any():
            return h
    try:
        sc = load_scores_history_cached_backend(
            sport=_canon_system_sport(sport), days_back=days_back,
            table_fq="sharplogger.sharp_data.game_scores_final", ttl_seconds=_PATHI_BIGAL_HISTORY_TTL_SECONDS
        )
    except Exception as e:
        logging.warning("⚠️ Pathi/BigAl final-score history unavailable: %s", e)
        return h
    if sc is None or sc.empty:
        return h
    sc = _ensure_merge_key_short_for_systems(sc)
    sc = sc.drop_duplicates("Merge_Key_Short", keep="last")
    add = [c for c in ["Merge_Key_Short", "Score_Home_Score", "Score_Away_Score", "Game_Start", "Home_Team", "Away_Team"] if c in sc.columns]
    for c in ["Score_Home_Score", "Score_Away_Score"]:
        if c in h.columns:
            h = h.drop(columns=[c], errors="ignore")
    h = h.merge(sc[add], on="Merge_Key_Short", how="left", suffixes=("", "__score"), validate="many_to_one")
    for base in ["Game_Start", "Home_Team", "Away_Team"]:
        scn = base + "__score"
        if scn in h.columns:
            if base not in h.columns:
                h[base] = h[scn]
            else:
                h[base] = h[base].combine_first(h[scn])
            h.drop(columns=[scn], inplace=True)
    return h
...
def fetch_pathi_bigal_history_backend(sport: str, days_back: int | None = _PATHI_BIGAL_DEFAULT_DAYS_BACK) -> pd.DataFrame:
    ...
    for table_fq in (
        ...
        try:
            candidate = _query_pathi_bigal_history_table(table_fq, canon, days_back)
            if candidate is not None and not candidate.empty:
                hist = candidate
                logging.info("✅ Pathi/BigAl history source=%s rows=%d sport=%s", table_fq, len(hist), canon)
                break
        except Exception as e:
            logging.warning("⚠️ Pathi/BigAl history source failed %s: %s", table_fq, e)

    if hist is None or hist.empty:
        _PATHI_BIGAL_HISTORY_CACHE[key] = (now, pd.DataFrame())
        return pd.DataFrame()

    hist = _attach_final_scores_to_system_history(hist, canon, days_back)
    hist = _ensure_merge_key_short_for_systems(hist)
    hist = enrich_bigal_context_from_bq(hist)
    if "Sport" in hist.columns:
        hist["Sport"] = canon
    _PATHI_BIGAL_HISTORY_CACHE[key] = (now, hist.copy())
    return hist
...
def attach_pathi_bigal_backend_features(current_rows: pd.DataFrame, sport: str | None, days_back: int | None = _PATHI_BIGAL_DEFAULT_DAYS_BACK) -> pd.DataFrame:
    ...
    original = enrich_bigal_context_from_bq(original)
    original = add_pathi_football_key_features(original)
    canon = _canon_system_sport(sport or (original["Sport"].iloc[0] if "Sport" in original.columns and len(original) else ""))
    if not canon:
        return original
    try:
        cur_sys = _physicalize_system_game_key(original)
        cur_sys["Sport"] = canon
        hist = fetch_pathi_bigal_history_backend(canon, days_back=days_back)
        hist_sys = _physicalize_system_game_key(hist) if hist is not None and not hist.empty else pd.DataFrame()
        if not hist_sys.empty:
            hist_sys["Sport"] = canon
            combo = pd.concat([hist_sys, cur_sys], ignore_index=True, sort=False)
        else:
            combo = cur_sys.copy()
        state = build_pathi_bigal_team_game_state(combo)
        current_keys = set(cur_sys["Game_Key"].dropna().astype(str).str.lower().str.strip())
        state_now = state[state["Game_Key"].astype(str).str.lower().str.strip().isin(current_keys)].copy() if not state.empty else state
        enriched = attach_pathi_bigal_features_to_market_rows(cur_sys, state_now)
        # Restore backend's line/outcome Game_Key exactly; keep Merge_Key_Short as system game id.
        if "__Backend_Line_Game_Key" in enriched.columns:
            enriched["Game_Key"] = enriched["__Backend_Line_Game_Key"].astype("string")
            enriched.drop(columns=["__Backend_Line_Game_Key"], inplace=True, errors="ignore")
        enriched["System_Feature_Version"] = PATHI_BIGAL_FEATURE_VERSION
        enriched.drop(columns=["Team"], inplace=True, errors="ignore")
        if len(enriched) != len(original):
            raise RuntimeError(f"system enrichment changed row count {len(original)} -> {len(enriched)}")
        return enriched
    except Exception as e:
        logging.warning("⚠️ Pathi/BigAl backend enrichment unavailable; legacy scoring preserved: %s", e, exc_info=True)
        return original
...
# ============================================================================
# V11.5.10 HISTORICAL BRAIN RUNTIME CONTRACT
# Mirrors the trained residual/multi-horizon historical expert exactly.
# ============================================================================
...
#
# Design rules:
#   * Historical Postgame_* fields are used only after shift/aggregation into
#     pregame state. Current-game box-score data never predicts that same game.
#   * The opening market remains the anchor. The statistical model corrects it;
#     it does not replace it.
#   * Protected latest-season shadow performance controls trust.
#   * Runtime scoring after the historical cutoff uses only saved prior profiles
#     plus information available before kickoff.
# ============================================================================
NCAAF_STAT_RAW_TABLE = "sharplogger.sharp_data.ncaaf_historical_game_side_raw"
NCAAF_STAT_FEATURE_VERSION = "2026-09-13-v13.2.28-market-residual-secondary-lane"
...
# ============================================================================
NCAAF_V13_VERSION = "2026-09-13-v13.2.28-independent-fair-market-residual-edge-architecture"
NCAAF_V13_HOTFIX = "V13_2_28__DYNAMIC_OFFDEF__ROBUST_MARGIN_ENSEMBLE__OOF_WEIGHTING__OWN_PROB_CALIBRATION__INTEGRATION_FIX"
...
_NCAAF_STAT_ADJ_METRICS = (
    "GameAdj_Off_YPP", "GameAdj_Def_YPP",
    "GameAdj_Off_Pass_YPA", "GameAdj_Def_Pass_YPA",
    "GameAdj_Off_Rush_YPA", "GameAdj_Def_Rush_YPA",
...
def _ncaaf_stat_safe_ratio(num, den):
    ...
    return (n / d.where(d.abs() > 1e-9)).replace([np.inf, -np.inf], np.nan)


def _ncaaf_stat_empirical_prob_gt(threshold, residuals):
    """P(residual > threshold) using a saved time-forward OOF residual sample."""
    ...
    if r.size < 30:
        return out
    ok = np.isfinite(thr)
    pos = np.searchsorted(r, thr[ok], side="right")
    out[ok] = 1.0 - (pos.astype(float) / float(r.size))
    # avoid exact 0/1 from a finite empirical sample
    eps = 0.5 / float(r.size + 1)
    return np.clip(out, eps, 1.0 - eps)


def _ncaaf_stat_logloss(y, p):
    ...
    if ok.sum() < 20 or np.unique(y[ok]).size < 2:
        return np.nan
    return float(log_loss(y[ok], np.clip(p[ok], 1e-5, 1-1e-5), labels=[0,1]))


def _ncaaf_stat_auc(y, p):
...
def _ncaaf_stat_new_margin_models():
    ...
    nonlinear = Pipeline([
        ...
        ("hgb", HistGradientBoostingRegressor(
            loss="squared_error", learning_rate=0.04, max_iter=140,
            max_leaf_nodes=15, max_depth=3, min_samples_leaf=25,
...
def _ncaaf_stat_blend_predict(models, X, linear_weight=0.75):
    lin, nonlin = models
    p1 = np.asarray(lin.predict(X), dtype=float)
    p2 = np.asarray(nonlin.predict(X), dtype=float)
...
def _ncaaf_stat_build_game_frame(raw: pd.DataFrame):
    ...
    numeric_cols = [
        ...
        "Consensus_Open_Spread","Consensus_Open_Total","Consensus_Open_Moneyline",
        "Postgame_First_Downs","Postgame_Total_Yards","Postgame_Total_Plays",
        "Postgame_Pass_Comp","Postgame_Pass_Att","Postgame_Pass_Yards",
        "Postgame_Rush_Att","Postgame_Rush_Yards","Postgame_Turnovers",
        "Q1_Points","Q2_Points","Q3_Points",
    ]
    for c in numeric_cols:
        if c not in r.columns: r[c] = np.nan
        r[c] = pd.to_numeric(r[c], errors="coerce").astype("float64")

    r["Off_YPP"] = _ncaaf_stat_safe_ratio(r["Postgame_Total_Yards"], r["Postgame_Total_Plays"])
    r["Off_Pass_YPA"] = _ncaaf_stat_safe_ratio(r["Postgame_Pass_Yards"], r["Postgame_Pass_Att"])
    r["Off_Rush_YPA"] = _ncaaf_stat_safe_ratio(r["Postgame_Rush_Yards"], r["Postgame_Rush_Att"])
    r["Off_Completion_Rate"] = _ncaaf_stat_safe_ratio(r["Postgame_Pass_Comp"], r["Postgame_Pass_Att"])
    ...
    off_cols = [c for c in _NCAAF_STAT_METRICS if c.startswith("Off_")]
    opp_actual = r[["Season","Source_Game_ID","Team_Norm"] + off_cols].copy()
    opp_actual = opp_actual.rename(columns={"Team_Norm":"Opponent_Norm", **{c:f"__OppActual_{c}" for c in off_cols}})
    r = r.merge(opp_actual, on=["Season","Source_Game_ID","Opponent_Norm"], how="left", validate="many_to_one")
    r["Def_YPP_Allowed"] = r["__OppActual_Off_YPP"]
    r["Def_Pass_YPA_Allowed"] = r["__OppActual_Off_Pass_YPA"]
    ...
    prev = prev.rename(columns={c:f"__PrevSeason_{c}" for c in _NCAAF_STAT_METRICS})
    r = r.merge(prev, on=["Season","Team_Norm"], how="left", validate="many_to_one")
    nat = summaries.groupby("Season", as_index=False)[list(_NCAAF_STAT_METRICS)].mean()
    nat["Season"] = nat["Season"] + 1
    nat = nat.rename(columns={c:f"__PrevNat_{c}" for c in _NCAAF_STAT_METRICS})
    r = r.merge(nat, on="Season", how="left", validate="many_to_one")

    state_cols=[]
    for c in _NCAAF_STAT_METRICS:
        season_prior = r.groupby(["Season","Team_Norm"], sort=False)[c].transform(
            lambda s: pd.to_numeric(s, errors="coerce").shift(1).expanding(min_periods=1).mean()
        )
        recent3 = r.groupby(["Season","Team_Norm"], sort=False)[c].transform(
            lambda s: pd.to_numeric(s, errors="coerce").shift(1).rolling(3, min_periods=1).mean()
        )
        prev_team = pd.to_numeric(r.get(f"__PrevSeason_{c}"), errors="coerce")
    ...
    opp_preg_cols = [
        ...
        "State_Def_Points_Per_Play_Allowed","State_Off_YPP","State_Off_Pass_YPA",
        "State_Off_Rush_YPA","State_Off_Points_Per_Play",
        "Q1_Points","Q2_Points","Q3_Points",
    ]
    opp_preg = r[["Season","Source_Game_ID","Team_Norm"] + [c for c in opp_preg_cols if c in r.columns]].copy()
    opp_preg = opp_preg.rename(columns={"Team_Norm":"Opponent_Norm", **{c:f"__OppPreg_{c}" for c in opp_preg_cols if c in r.columns}})
    r = r.merge(opp_preg, on=["Season","Source_Game_ID","Opponent_Norm"], how="left", validate="many_to_one")

    q3_team = pd.to_numeric(r.get("Q1_Points"),errors="coerce").fillna(0) + pd.to_numeric(r.get("Q2_Points"),errors="coerce").fillna(0) + pd.to_numeric(r.get("Q3_Points"),errors="coerce").fillna(0)
    q3_opp = pd.to_numeric(r.get("__OppPreg_Q1_Points"),errors="coerce").fillna(0) + pd.to_numeric(r.get("__OppPreg_Q2_Points"),errors="coerce").fillna(0) + pd.to_numeric(r.get("__OppPreg_Q3_Points"),errors="coerce").fillna(0)
    q3_margin = (q3_team-q3_opp).abs()
    final_margin = (pd.to_numeric(r["Team_Score"],errors="coerce")-pd.to_numeric(r["Opponent_Score"],errors="coerce")).abs()
    comp_w = pd.Series(np.where(q3_margin.ge(28),0.65,np.where(final_margin.ge(28),0.80,1.0)),index=r.index,dtype="float64")
    r["Competitive_Game_Weight"] = comp_w

    adj_defs = {
        "GameAdj_Off_YPP": ("Off_YPP","__OppPreg_State_Def_YPP_Allowed",+1),
        "GameAdj_Def_YPP": ("__OppPreg_State_Off_YPP","Def_YPP_Allowed",+1),
        "GameAdj_Off_Pass_YPA": ("Off_Pass_YPA","__OppPreg_State_Def_Pass_YPA_Allowed",+1),
        "GameAdj_Def_Pass_YPA": ("__OppPreg_State_Off_Pass_YPA","Def_Pass_YPA_Allowed",+1),
        "GameAdj_Off_Rush_YPA": ("Off_Rush_YPA","__OppPreg_State_Def_Rush_YPA_Allowed",+1),
        "GameAdj_Def_Rush_YPA": ("__OppPreg_State_Off_Rush_YPA","Def_Rush_YPA_Allowed",+1),
        "GameAdj_Off_Points_Per_Play": ("Off_Points_Per_Play","__OppPreg_State_Def_Points_Per_Play_Allowed",+1),
        "GameAdj_Def_Points_Per_Play": ("__OppPreg_State_Off_Points_Per_Play","Def_Points_Per_Play_Allowed",+1),
    }
    ...
    adj_prev=adj_prev.rename(columns={c:f"__PrevSeason_{c}" for c in _NCAAF_STAT_ADJ_METRICS})
    r=r.merge(adj_prev,on=["Season","Team_Norm"],how="left",validate="many_to_one")
    adj_nat=adj_summaries.groupby("Season",as_index=False)[list(_NCAAF_STAT_ADJ_METRICS)].mean(); adj_nat["Season"]=adj_nat["Season"]+1
    adj_nat=adj_nat.rename(columns={c:f"__PrevNat_{c}" for c in _NCAAF_STAT_ADJ_METRICS})
    r=r.merge(adj_nat,on="Season",how="left",validate="many_to_one")
    for c in _NCAAF_STAT_ADJ_METRICS:
        season_prior=r.groupby(["Season","Team_Norm"],sort=False)[c].transform(lambda s: pd.to_numeric(s,errors="coerce").shift(1).expanding(min_periods=1).mean())
        recent3=r.groupby(["Season","Team_Norm"],sort=False)[c].transform(lambda s: pd.to_numeric(s,errors="coerce").shift(1).rolling(3,min_periods=1).mean())
    ...
    anchor["Market_Open_Total"] = pd.to_numeric(anchor["Consensus_Open_Total"], errors="coerce")

    # opening ML fair probability for anchor side when both prices exist
    def _amer(o):
        o=pd.to_numeric(o,errors="coerce"); p=pd.Series(np.nan,index=o.index,dtype=float)
        neg=o<0; pos=o>0
        p.loc[neg]=(-o.loc[neg])/((-o.loc[neg])+100.0); p.loc[pos]=100.0/(o.loc[pos]+100.0)
        return p
...
def fit_ncaaf_statistical_brain(log_func=print):
    ...
    if isinstance(_NCAAF_STAT_TRAIN_CACHE.get("bundle"), dict):
        ...
        return b
    try:
        raw = bq_client.query(f"""
          SELECT * FROM `{NCAAF_STAT_RAW_TABLE}`
    ...
    for val_season in seasons[1:]:
        ...
        ct=np.where(np.isfinite(market_t),(1-market_weight)*market_t+market_weight*stot,stot)
        oof_margin[np.where(va)[0]]=cm; oof_total[np.where(va)[0]]=ct
    ...
    ysp=(spread_margin[valid_sp]>0).astype(int)
    psp=_ncaaf_stat_empirical_prob_gt(-(smarg[valid_sp]+shadow_open_sp[valid_sp]),resid_m)
    ll_sp=_ncaaf_stat_logloss(ysp,psp); auc_sp=_ncaaf_stat_auc(ysp,psp); br_sp=_ncaaf_stat_brier(ysp,psp)

    total_diff=at-shadow_open_tot
    valid_tot=np.isfinite(total_diff)&~np.isclose(total_diff,0,atol=1e-9)&np.isfinite(stotl)&np.isfinite(shadow_open_tot)
    ytot=(total_diff[valid_tot]>0).astype(int)
    ...
    def _trust(ll,auc,market_ll=0.69314718056):
        ...
        a=max(0.0,(auc-0.5)/0.06) if np.isfinite(auc) else 0.0
        return float(np.clip(0.65*(ll_skill/0.012)+0.35*a,0,1))
    trust_sp=_trust(ll_sp,auc_sp)
...
def _ncaaf_stat_runtime_baseline(df, market):
    ...
    if m in ("spreads","totals"):
        return pd.Series(0.5,index=idx,dtype=float)
    odds=None
    for c in ("ML_Fair_Prob","Market_Implied_Prob","Implied_Prob"):
        if c in df.columns:
            s=pd.to_numeric(df[c],errors="coerce")
            if s.notna().any(): odds=s; break
    if odds is not None: return odds.clip(.01,.99).fillna(.5)
    for c in ("Odds_Price","ML_Odds"):
        if c in df.columns:
            o=pd.to_numeric(df[c],errors="coerce"); p=pd.Series(np.nan,index=idx,dtype=float); neg=o<0; pos=o>0
            p.loc[neg]=(-o.loc[neg])/((-o.loc[neg])+100); p.loc[pos]=100/(o.loc[pos]+100)
            return p.clip(.01,.99).fillna(.5)
...
def _ncaaf_stat_runtime_frame(df: pd.DataFrame, sb: dict):
    ...
    if not isinstance(profiles,pd.DataFrame) or profiles.empty:
        return out
    p=profiles.copy(); p["Team_Norm"]=p["Team_Norm"].astype(str).str.lower().str.strip(); p=p.drop_duplicates("Team_Norm",keep="last").set_index("Team_Norm")
    home=df.get("Home_Team_Norm",df.get("Home_Team",pd.Series("",index=df.index))).astype(str).str.lower().str.strip()
...
def _ncaaf_stat_profile_similarity(X, sb):
    ...
    a=X.apply(pd.to_numeric,errors="coerce").to_numpy(dtype=float,na_value=np.nan)
    if med.size!=a.shape[1] or scale.size!=a.shape[1] or a.shape[1]==0: return np.ones(len(X),dtype=float)
    scale=np.where(np.isfinite(scale)&(scale>1e-6),scale,1.0); valid=np.isfinite(a)&np.isfinite(med)[None,:]
    z=np.abs((a-med[None,:])/scale[None,:]); z=np.where(valid,np.minimum(z,8),np.nan)
    with np.errstate(all="ignore"): meanz=np.nanmean(z,axis=1)
    cov=valid.mean(axis=1); meanz=np.where(np.isfinite(meanz),meanz,4)
    return np.clip(np.exp(-.18*meanz)*(.60+.40*cov),.10,1.0)


# ============================================================================
# V13 NCAAF VALUE ARCHITECTURE — RUNTIME SHADOW SCORER
# ============================================================================
def _ncaaf_v13_pair_predict(models, X, weight=0.80):
...
def _ncaaf_v13_break_even_prob(odds):
    o=pd.to_numeric(pd.Series(odds),errors="coerce").to_numpy(dtype=float,na_value=np.nan)
    p=np.full(len(o),np.nan,dtype=float); neg=o<0; pos=o>0
    p[neg]=(-o[neg])/((-o[neg])+100.0); p[pos]=100.0/(o[pos]+100.0)
    return p


def _ncaaf_v13_profit_per_dollar(odds):
    o=pd.to_numeric(pd.Series(odds),errors="coerce").to_numpy(dtype=float,na_value=np.nan)
    prof=np.full(len(o),np.nan,dtype=float); neg=o<0; pos=o>0
    prof[neg]=100.0/(-o[neg]); prof[pos]=o[pos]/100.0
    return prof


def _ncaaf_v13_market_runtime_features(rows: pd.DataFrame):
    """Build book-aware current market state at the physical game grain, then map to rows."""
    d=rows.copy()
    idx=d.index
    home=d.get("Home_Team_Norm",d.get("Home_Team",pd.Series("",index=idx))).astype(str).str.lower().str.strip()
    away=d.get("Away_Team_Norm",d.get("Away_Team",pd.Series("",index=idx))).astype(str).str.lower().str.strip()
    outcome=d.get("Outcome_Norm",d.get("Outcome",pd.Series("",index=idx))).astype(str).str.lower().str.strip()
    value=pd.to_numeric(d.get("Value",np.nan),errors="coerce")
    openv=None
    for c in ("First_Line_Value","Open_Value","Opening_Line","Opening_Spread"):
        if c in d.columns:
            ss=pd.to_numeric(d[c],errors="coerce")
            if ss.notna().any(): openv=ss; break
    if openv is None: openv=pd.Series(np.nan,index=idx,dtype=float)
    is_home=outcome.eq(home)|outcome.eq("home"); is_away=outcome.eq(away)|outcome.eq("away")
    d["__v13_home_spread"]=np.where(is_home,value,np.where(is_away,-value,np.nan))
    d["__v13_open_home_spread"]=np.where(is_home,openv,np.where(is_away,-openv,np.nan))
    book=d.get("Bookmaker",d.get("Book",pd.Series("",index=idx))).astype(str).str.lower().str.strip()
    d["__v13_book"]=book
    sharp_set={str(x).lower().strip() for x in SHARP_BOOKS}; rec_set={str(x).lower().strip() for x in REC_BOOKS}
    d["__v13_sharp"]=book.isin(sharp_set); d["__v13_rec"]=book.isin(rec_set)
    if "Game_Key" in d.columns:
        g=d["Game_Key"].astype(str).str.lower().str.strip()
        bad=g.eq("")|g.eq("nan")|g.eq("none")
    else:
        g=pd.Series("",index=idx); bad=pd.Series(True,index=idx)
    gs=pd.to_datetime(d.get("Game_Start",pd.NaT),errors="coerce",utc=True)
    fallback=home+"|"+away+"|"+gs.dt.strftime("%Y-%m-%dT%H:%M").fillna("")
    d["__v13_group"]=g.where(~bad,fallback)
    snap=pd.to_datetime(d.get("Snapshot_Timestamp",pd.NaT),errors="coerce",utc=True)
    hours=(gs-snap).dt.total_seconds().div(3600.0)
    d["__v13_hours"]=hours

    feat=pd.DataFrame(index=idx)
    for _,gx in d.groupby("__v13_group",sort=False):
        gi=gx.index
        hm=-pd.to_numeric(gx["__v13_home_spread"],errors="coerce")
        om=-pd.to_numeric(gx["__v13_open_home_spread"],errors="coerce")
        sm=-pd.to_numeric(gx.loc[gx["__v13_sharp"],"__v13_home_spread"],errors="coerce")
        rm=-pd.to_numeric(gx.loc[gx["__v13_rec"],"__v13_home_spread"],errors="coerce")
        current=float(hm.median()) if hm.notna().any() else np.nan
        op=float(om.median()) if om.notna().any() else current
        sh=float(sm.median()) if sm.notna().any() else current
        rc=float(rm.median()) if rm.notna().any() else current
        hh=float(pd.to_numeric(gx["__v13_hours"],errors="coerce").median()) if pd.to_numeric(gx["__v13_hours"],errors="coerce").notna().any() else 6.0
        hgrid=np.asarray(NCAAF_V13_HORIZONS_HOURS,dtype=float)
        huse=float(hgrid[np.argmin(np.abs(hgrid-hh))]) if np.isfinite(hh) else 6.0
        vals={
            "Current_Margin":current,"Open_Margin":op,"Sharp_Current_Margin":sh,"Rec_Current_Margin":rc,
            "SharpMinusRec":sh-rc,"Move_From_Open":current-op,"Sharp_Move_From_Open":sh-op,"Rec_Move_From_Open":rc-op,
            "Book_Dispersion":float(hm.std(ddof=0)) if hm.notna().sum()>1 else 0.0,
...
def _ncaaf_v13_runtime_week_bucket(v):
    try: x=float(v)
...
def _ncaaf_v13_runtime_games_prior_bucket(v):
    try: x=float(v)
    except Exception: return "UNKNOWN"
    if not np.isfinite(x): return "UNKNOWN"
...
def _ncaaf_v13_runtime_early_situational_features(out: pd.DataFrame, Xstate: pd.DataFrame):
    """Reconstruct the exact pregame side-level Games 1-2 situational feature schema."""
    ...
    gp=num("BigAl_Context_Team_Game_Number","Team_Game_Number")-1.0
    if gp.notna().sum()==0:
        gp=pd.to_numeric(Xstate.get("__Stat_Current_Season_Games",np.nan),errors="coerce")
        if not isinstance(gp,pd.Series): gp=pd.Series(gp,index=out.index,dtype=float)
    x["ES_Games_Prior"]=gp.clip(lower=0)
    x["ES_Is_Home"]=num("BigAl_Context_Is_Home","Is_Home").fillna(0.0)
    x["ES_Is_Neutral"]=num("Is_Neutral_Site","Is_Neutral").fillna(0.0)
    sp=num("First_Line_Value","Opening_Spread","Open_Value","Value")
    x["ES_Open_Spread"]=sp; x["ES_Abs_Spread"]=sp.abs(); x["ES_Is_Favorite"]=np.where(sp.notna(),(sp<0).astype(float),np.nan)
    hs=out.get("Home_Subdivision",pd.Series("",index=out.index)).astype(str).str.upper().str.strip()
    ass=out.get("Away_Subdivision",pd.Series("",index=out.index)).astype(str).str.upper().str.strip()
    sk=hs.ne("")&ass.ne(""); x["ES_Cross_Subdivision"]=np.where(sk,hs.ne(ass).astype(float),np.nan)
    conf=num("BigAl_Context_Is_Conference_Game","Is_Conference_Game")
    x["ES_NonConference"]=np.where(conf.notna(),1.0-conf,np.nan)
    x["ES_Prev_SU_Margin"]=num("Prev_SU_Margin")
    x["ES_Prev_ATS_Margin"]=num("Prev_ATS_Cover_Margin","Prev_ATS_Margin")
    x["ES_Prev_Points_For"]=num("Prev_Points_For")
    x["ES_Prev_SU_Win"]=np.where(x["ES_Prev_SU_Margin"].notna(),(x["ES_Prev_SU_Margin"]>0).astype(float),num("Prev_SU_Win"))
    x["ES_Prev_SU_Loss"]=np.where(x["ES_Prev_SU_Margin"].notna(),(x["ES_Prev_SU_Margin"]<0).astype(float),num("Prev_SU_Loss"))
    x["ES_Prev_BigWin21"]=np.where(x["ES_Prev_SU_Margin"].notna(),(x["ES_Prev_SU_Margin"]>=21).astype(float),np.nan)
    x["ES_Prev_BigLoss21"]=np.where(x["ES_Prev_SU_Margin"].notna(),(x["ES_Prev_SU_Margin"]<=-21).astype(float),np.nan)
    x["ES_Prev_BigCover10"]=np.where(x["ES_Prev_ATS_Margin"].notna(),(x["ES_Prev_ATS_Margin"]>=10).astype(float),np.nan)
    x["ES_Prev_BigMiss10"]=np.where(x["ES_Prev_ATS_Margin"].notna(),(x["ES_Prev_ATS_Margin"]<=-10).astype(float),np.nan)
    x["Opp_ES_Prev_SU_Win"]=num("Opp_Prev_SU_Win")
    x["Opp_ES_Prev_SU_Loss"]=num("Opp_Prev_SU_Loss")
    x["Opp_ES_Prev_Points_For"]=num("Opp_Prev_Points_For")
    x["ES_Prev_ATS_x_Favorite"]=x["ES_Prev_ATS_Margin"]*x["ES_Is_Favorite"]
    x["ES_Prev_SU_x_AbsSpread"]=x["ES_Prev_SU_Margin"]*x["ES_Abs_Spread"]
    x["ES_CrossSub_x_AbsSpread"]=x["ES_Cross_Subdivision"]*x["ES_Abs_Spread"]
    return x.replace([np.inf,-np.inf],np.nan)


def _ncaaf_v13_empirical_tail_prob(residuals, thresholds):
    rr=np.sort(np.asarray(residuals,dtype=float)); rr=rr[np.isfinite(rr)]
    th=np.asarray(thresholds,dtype=float)
    if len(rr)==0: return np.full(len(th),np.nan)
    pos=np.searchsorted(rr,th,side="right")
    # Jeffreys-style finite sample smoothing.
    return np.clip((len(rr)-pos+0.5)/(len(rr)+1.0),1e-4,1-1e-4)


def _ncaaf_v13_promoted_empirical_probability(preview, thresholds, games_prior, side_orientation=None):
    """Empirical cover probability with exact side-symmetry at runtime.

    Residuals in the V13 bundle are stored in the training anchor/home orientation.
    For the opposite side the residual distribution must be reflected (R -> -R),
    otherwise canonical and inverse rows need not be true complements.
    """
    gp=np.asarray((preview or {}).get("global_residuals") or [],dtype=float)
    th=np.asarray(thresholds,dtype=float)
    if side_orientation is None:
        out=np.asarray(_ncaaf_v13_empirical_tail_prob(gp,th),dtype=float)
    else:
        orient=np.asarray(side_orientation,dtype=float)
        out=np.full(len(th),np.nan,dtype=float)
        pos=np.isfinite(orient)&(orient>0)&np.isfinite(th)
        neg=np.isfinite(orient)&(orient<0)&np.isfinite(th)
        if pos.any():
            out[pos]=np.asarray(_ncaaf_v13_empirical_tail_prob(gp,th[pos]),dtype=float)
        if neg.any():
            out[neg]=np.asarray(_ncaaf_v13_empirical_tail_prob(-gp,th[neg]),dtype=float)
    buckets=np.asarray([_ncaaf_v13_runtime_games_prior_bucket(v) for v in games_prior],dtype=object)
    scale=float(np.clip((preview or {}).get("drift_scale",1.0) or 1.0,1.0,1.5))
    # Cold-start Games 1-2 are a known maturity regime, not a reason to learn a
    # global drift multiplier from a small opening slate. Drift is applied only
    # once both teams have moved beyond the first two games.
    mature=~np.isin(buckets,["FIRST_TWO_GAMES","UNKNOWN"])
    out[mature]=0.5+(out[mature]-0.5)/scale
    return np.clip(out,0.01,0.99),buckets,scale


V13_SPECIALIST_OUTPUT_COLS = [
    "V13_Own_Fair_Margin","V13_Own_Cover_Prob","V13_Own_Push_Prob",
    "V13_Market_Margin","V13_Market_BreakEven_Prob","V13_Market_NoVig_Prob",
    "V13_Point_Edge","V13_Raw_Probability_Edge","V13_Market_Fair_Probability_Edge","V13_Own_BreakEven_Edge",
    "V13_Final_BreakEven_Edge","V13_Final_EV_Per_Dollar","V13_MarketResidual_Prob","V13_Final_Fair_Prob",
    ...
    "V13_Fundamental_Overlay_Active","V13_Fundamental_Overlay_Contribution","V13_Fundamental_Overlay_Regime",
...
def _v13_overlay_logit(p):
    a=np.clip(np.asarray(p,dtype=float),1e-5,1-1e-5)
...
def _v131_runtime_numeric_series(rows: pd.DataFrame, names, default=np.nan):
    if rows is None: return pd.Series(dtype=float)
    for c in names:
        if c in rows.columns:
            z=pd.to_numeric(rows[c],errors="coerce")
            if isinstance(z,pd.Series): return z.reindex(rows.index)
    return pd.Series(default,index=rows.index,dtype=float)


def _v131_runtime_family_regime_labels(rows: pd.DataFrame, family: str, maturity=None):
    n=0 if rows is None else len(rows)
    if n==0: return np.asarray([],dtype=object)
    fam=str(family); mat=np.asarray(maturity if maturity is not None else ["UNKNOWN"]*n,dtype=object)
    if fam=="Fundamental": return mat.astype(object)
    spread=_v131_runtime_numeric_series(rows,("Value","Spread_Value","Current_Spread","Opening_Spread")); sv=spread.to_numpy(dtype=float,na_value=np.nan); a=np.abs(sv); key=np.zeros(n,dtype=bool)
    for k in (3.0,7.0,10.0,14.0): key |= np.isfinite(a)&(np.abs(a-k)<=0.75)
    for c in ("Crossed_Key_3","Crossed_Key_7","Crossed_Key_10","Crossed_Key_14","Pathi_FB_Moved_Through_Key","Pathi_FB_Crossed_Key_Toward_Team","Pathi_FB_Crossed_Key_Away_From_Team","Pathi_FB_Moved_Onto_Key","Pathi_FB_Moved_Off_Key"):
        if c in rows.columns: key |= pd.to_numeric(rows[c],errors="coerce").fillna(0).to_numpy(dtype=float)>0
    if fam=="Market":
        move=np.zeros(n,dtype=bool)
        for c in ("Line_Move_30m","Line_Move_60m","Line_Move_120m","Line_Move_Last30m","Line_Move_Last60m","Line_Move_From_Open","Sharp_Book_Move_30m","Sharp_Book_Move_60m","Sharp_Move_Before_Market","Sharp_Soft_Divergence","OddsMove_Magnitude","SharpMove_Magnitude"):
            if c in rows.columns: move |= np.abs(pd.to_numeric(rows[c],errors="coerce").fillna(0).to_numpy(dtype=float))>1e-6
        lab=np.full(n,"QUIET",dtype=object); lab[move]="ACTIVE_MOVE"; lab[key]="KEY_ZONE"; lab[key&move]="KEY_MOVE"; return lab
    if fam=="Pathi":
        active=np.zeros(n,dtype=bool)
        for c in rows.columns:
            sl=str(c).lower()
            if sl.startswith("pathi_") and any(tok in sl for tok in ("cross","key","hook","dog","favorite","role","protected","move")):
                z=pd.to_numeric(rows[c],errors="coerce"); active |= z.fillna(0).abs().to_numpy(dtype=float)>1e-9
        lab=np.full(n,"OTHER",dtype=object); lab[active]="SYSTEM_ACTIVE"; lab[key]="KEY_NUMBER"; return lab
    if fam=="BigAl":
...
def _apply_v131_core_calibration_runtime(core_prob, art: dict, maturity=None):
    """Apply frozen V13 core calibration and validated maturity temperatures."""
    ...
    def raw_map(q):
        ...
        if method=="temperature": return _v13_overlay_sigmoid(_v13_overlay_logit(q)/max(float(rec.get("temperature",1.0) or 1.0),1e-6))
        if method=="platt":
            c=list(rec.get("coef") or [1.0]); return _v13_overlay_sigmoid(float(rec.get("intercept",0.0) or 0.0)+float(c[0])*_v13_overlay_logit(q))
        if method=="beta":
            c=list(rec.get("coef") or [1.0,1.0]); c=c if len(c)>=2 else [1.0,1.0]; return _v13_overlay_sigmoid(float(rec.get("intercept",0.0) or 0.0)+float(c[0])*np.log(q)+float(c[1])*(-np.log1p(-q)))
        return q
    if valid.any():
        q=np.clip(raw[valid],1e-5,1-1e-5)
        try:
            fq=np.asarray(raw_map(q),dtype=float)
            if method in {"platt","beta"}: fq=0.5*(fq+(1.0-np.asarray(raw_map(1.0-q),dtype=float)))
            final[valid]=fq
        except Exception as e:
            logging.warning("V13.1 core calibration runtime failed closed to identity: %s",e); final[valid]=q; method="identity"
    temps=(art.get("maturity_temperatures") or {}) if isinstance(art,dict) else {}; buckets=np.asarray(maturity if maturity is not None else ["UNKNOWN"]*len(final),dtype=object); regime_temp=np.ones(len(final),dtype=float)
    if isinstance(temps,dict):
        for b,t in temps.items():
            try: tv=float(t)
            except Exception: continue
            m=(buckets.astype(str)==str(b))&np.isfinite(final)
            if m.any() and np.isfinite(tv) and abs(tv-1)>1e-12:
                final[m]=_v13_overlay_sigmoid(_v13_overlay_logit(final[m])/tv); regime_temp[m]=tv
    final=np.clip(final,0.01,0.99); active=valid&((method!="identity")|(np.abs(regime_temp-1.0)>1e-12))
    return final,{"method":method,"active":active.astype(np.int8),"contribution":np.asarray(final-raw,dtype=float),"maturity_temperature":regime_temp}
...
def _v132_runtime_rule_trigger(rows: pd.DataFrame, spec: dict) -> np.ndarray:
    ...
    for c in list(spec.get("columns") or []):
        if c not in rows.columns: return np.zeros(n,dtype=bool)
    ...
    if spec.get("kind")=="column_role":
        ...
        role=str(spec.get("role") or "").upper(); m &= (spread>0) if role=="DOG" else ((spread<0) if role=="FAVORITE" else np.ones(n,dtype=bool))
    return m
...
def _v13224_runtime_resolver_matrix(core_cal, base, market, pathi, bigal, feature_names=None):
    ...
    new=pd.DataFrame({
        ...
        "MarketMicrostructure_Delta_Logit":zm-zr,
        "Pathi_Delta_Logit":zp-zm,
        "BigAl_Delta_Logit":zbi-zm,
    })
    # Exact backward replay for saved <=13.2.26 resolver artifacts.
    feats=list(feature_names or [])
    if feats and any(f in feats for f in ("Fundamental_Delta_Logit","Market_Delta_Logit")):
        return pd.DataFrame({
            "Fundamental_Delta_Logit":zr-zo,
            "Market_Delta_Logit":zm-zr,
            "Pathi_Delta_Logit":zp-zm,
            "BigAl_Delta_Logit":zbi-zm,
        })
    return new
...
def _v13227_runtime_empirical_cover_probability(residuals, threshold):
    rr=np.asarray(residuals,dtype=float); rr=rr[np.isfinite(rr)]
    th=np.asarray(threshold,dtype=float); p=np.full(len(th),np.nan); q=np.full(len(th),np.nan)
    if len(rr)<50: return p,q
    for i,t in enumerate(th):
        if not np.isfinite(t): continue
        win=float(np.mean(rr>t+1e-9)); push=float(np.mean(np.isclose(rr,t,atol=1e-9,rtol=0.0))); lose=max(0.0,1-win-push)
        p[i]=win/(win+lose) if win+lose>1e-12 else 0.5; q[i]=push
    return np.clip(p,0.01,0.99),np.clip(q,0.0,1.0)


def _v13228_runtime_apply_prob_calibrator(p, cal):
    x=np.asarray(p,dtype=float)
    out=np.full(len(x),np.nan,dtype=float)
    ok=np.isfinite(x)
    if not ok.any():
        return out
    if not bool((cal or {}).get("active",False)):
        out[ok]=np.clip(x[ok],0.01,0.99); return out
    z=np.log(np.clip(x[ok],1e-5,1-1e-5)/np.clip(1.0-x[ok],1e-5,1.0))
    z2=float((cal or {}).get("intercept",0.0))+float((cal or {}).get("slope",1.0))*z
    out[ok]=np.clip(1.0/(1.0+np.exp(-np.clip(z2,-30,30))),0.01,0.99)
    return out


def _v13228_runtime_dynamic_margin(rows: pd.DataFrame, spec: dict):
    """Reconstruct the saved sequential offense/defense fair margin for live games."""
    n=0 if rows is None else len(rows)
    out=np.full(n,np.nan,dtype=float)
    if rows is None or n==0 or not isinstance(spec,dict):
        return out
    off={str(k).lower().strip():float(v) for k,v in (spec.get("offense_state") or {}).items()}
    deff={str(k).lower().strip():float(v) for k,v in (spec.get("defense_state") or {}).items()}
    par=spec.get("params") or {}
    hfa=float(par.get("hfa",2.5))
    ...
    neutral=pd.Series(False,index=rows.index)
    for c in ("Is_Neutral","Neutral_Site","Context_Is_Neutral"):
        if c in rows.columns:
            vv=pd.to_numeric(rows[c],errors="coerce").fillna(0)
            neutral=neutral|vv.ge(0.5)
    for i,(h,a,neu) in enumerate(zip(home,away,neutral)):
        if not h or not a:
            continue
        oh=float(off.get(h,0.0)); oa=float(off.get(a,0.0))
        dh=float(deff.get(h,0.0)); da=float(deff.get(a,0.0))
        out[i]=(oh-da)-(oa-dh)+(0.0 if bool(neu) else hfa)
    return out


def _v13228_runtime_own_fair_margin(rows: pd.DataFrame, Xstate: pd.DataFrame, own: dict):
    """V13.2.28 market-blind ensemble prediction with saved OOF-selected weights."""
    n=len(rows)
    cols=list((own or {}).get("margin_features") or [])
    models=(own or {}).get("margin_models")
    if not cols or models is None:
        return np.full(n,np.nan,dtype=float)
    xx=Xstate.reindex(columns=cols)
    try:
        ridge=np.asarray(models[0].predict(xx),dtype=float)
        hgb=np.asarray(models[1].predict(xx),dtype=float)
    except Exception:
        return np.full(n,np.nan,dtype=float)
...
def _v13227_runtime_novig_spread_prob(rows: pd.DataFrame, break_even):
    """Paired-side no-vig market probability when both prices are truly present."""
    d=rows.reset_index(drop=True); be=np.asarray(break_even,dtype=float); out=np.full(len(d),np.nan,dtype=float)
    if len(d)==0: return out
    game_col=next((c for c in ("Merge_Key_Short","Game_Key","feat_Game_Key","Source_Game_ID") if c in d.columns),None)
    book_col=next((c for c in ("Bookmaker","Book","Bookmaker_Norm","Book_Norm") if c in d.columns),None)
    time_col=next((c for c in ("Snapshot_Timestamp","Time","Captured_At","Observed_At") if c in d.columns),None)
    if game_col is None or book_col is None: return out
    raw_val=pd.to_numeric(d.get("Value",pd.Series(np.nan,index=d.index)),errors="coerce")
    val=raw_val.abs().round(4)
    key=pd.DataFrame({"g":d[game_col].astype(str),"b":d[book_col].astype(str),"v":val})
    if time_col is not None:
        key["t"]=pd.to_datetime(d[time_col],errors="coerce",utc=True).astype(str)
    elif "Horizon_Hours" in d.columns:
        key["t"]=pd.to_numeric(d["Horizon_Hours"],errors="coerce").round(3).astype(str)
    else:
        key["t"]=""
    ks=key.astype(str).agg("|".join,axis=1)
    tmp=pd.DataFrame({"k":ks,"be":be})
    outcome=d.get("Outcome_Norm",d.get("Outcome",pd.Series("",index=d.index))).astype(str).str.lower().str.strip()
    for _,z in tmp.groupby("k",sort=False):
        ix=z.index.to_numpy(dtype=int); good=ix[np.isfinite(be[ix])&np.isfinite(raw_val.to_numpy(dtype=float,na_value=np.nan)[ix])]
        if len(good)!=2: continue
        vv=raw_val.iloc[good].to_numpy(dtype=float)
        # A paired spread market must represent opposite sides of the same number.
        if not (np.isfinite(vv).all() and abs(float(vv[0]+vv[1]))<=0.02 and np.sign(vv[0])!=np.sign(vv[1])): continue
        oo=outcome.iloc[good].astype(str).to_numpy(dtype=object)
        if oo[0] and oo[1] and oo[0]==oo[1]: continue
        den=float(np.sum(be[good]))
...
def _apply_v13_specialist_overlays_runtime(rows: pd.DataFrame, base_prob, overlay: dict, maturity_bucket, core_calibrated_prob=None):
    """V13.2.28 own-fair-anchored residual probability resolver with fail-closed authority routing."""
...
def apply_ncaaf_v13_shadow(rows: pd.DataFrame, bundle: dict):
    ...
    defaults={
        ...
        "V13_Own_Fair_Margin":np.nan,"V13_Own_Cover_Prob":np.nan,"V13_Own_Push_Prob":np.nan,
        "V13_Market_Margin":np.nan,"V13_Market_BreakEven_Prob":np.nan,"V13_Market_NoVig_Prob":np.nan,
        "V13_Point_Edge":np.nan,"V13_Raw_Probability_Edge":np.nan,"V13_Market_Fair_Probability_Edge":np.nan,
        "V13_Own_BreakEven_Edge":np.nan,"V13_Final_BreakEven_Edge":np.nan,"V13_Final_EV_Per_Dollar":np.nan,
        "V13_MarketResidual_Prob":np.nan,"V13_Final_Fair_Prob":np.nan,
    ...
    _v13_runtime_enabled=str(os.getenv("V13_PROMOTION_ENABLED","1")).strip().lower() not in {"0","false","no","off"}
    if isinstance(preview,dict) and preview.get("enabled") and not _v13_runtime_enabled:
        preview=dict(preview); preview["enabled"]=False
    if not isinstance(fund,dict) or not fund.get("margin_features"): return out
    try:
        ...
        # V13.2.28: they predict points of market error, not OUR fair margin.
        ...
        _own_cols=list(_own.get("margin_features") or [])
        own_fair_home=_v13228_runtime_own_fair_margin(out,Xstate,_own) if _own.get("margin_models") is not None and _own_cols else np.full(n,np.nan,dtype=float)
        ...
        own_fair_side=np.asarray(own_fair_home,dtype=float)*orient
        _residual_side=np.asarray(_market_residual_home,dtype=float)*orient
        # Market Residual Brain is a delta around the observed market number.
        raw_fair_side=np.where(np.isfinite(offered_margin),offered_margin+_residual_side,np.nan)
        raw_fair_home=raw_fair_side*orient
        ...
        maturity_bucket=np.asarray([_ncaaf_v13_runtime_games_prior_bucket(v) for v in curg],dtype=object)
        ...
        if isinstance(preview,dict) and preview.get("enabled"):
            _mb=(preview.get("maturity_betas") or {})
            for _b in ("FIRST_TWO_GAMES","GAMES_3_4","GAMES_5_8","GAME_9_PLUS","UNKNOWN"):
                _m=maturity_bucket==_b
                if _m.any(): beta_arr[_m]=float(np.clip(_mb.get(_b,beta_global),0.0,1.0))
        tradable_fair_side=np.where(np.isfinite(offered_margin), offered_margin+beta_arr*(raw_fair_side-offered_margin), raw_fair_side)
        ...
        # V13.2.28 parity fallback for the secondary Market-Residual Brain.  The
        ...
        # Older preview/calibrator artifacts remain preferred when available, but
        # a missing preview can no longer silently turn a validated residual expert
        # into an all-NaN opinion at runtime.
        _market_resid_pool=np.asarray(fund.get("residual_margin",[]),dtype=float)
        _market_residual_prob_direct=np.full(n,np.nan,dtype=float)
        _market_residual_push_direct=np.full(n,np.nan,dtype=float)
        if len(_market_resid_pool)>=50:
            _rthr=offered_margin-tradable_fair_side
            if len(_ih):
                _market_residual_prob_direct[_ih],_market_residual_push_direct[_ih]=_v13227_runtime_empirical_cover_probability(_market_resid_pool,_rthr[_ih])
            if len(_ia):
                _market_residual_prob_direct[_ia],_market_residual_push_direct[_ia]=_v13227_runtime_empirical_cover_probability(-_market_resid_pool,_rthr[_ia])
        ...
        raw_fair_total=np.where(np.isfinite(ct),ct+np.asarray(_market_residual_total,dtype=float),np.asarray(_market_residual_total,dtype=float)); tradable_fair_total=np.where(np.isfinite(ct),ct+tbeta*(raw_fair_total-ct),raw_fair_total)
        ...
        prob=np.full(n,np.nan,dtype=float); drift_scale=1.0
        early_sit_prob=np.full(n,np.nan,dtype=float); early_sit_active=np.zeros(n,dtype=bool)
        if isinstance(preview,dict) and preview.get("enabled"):
            _thr=offered_margin-tradable_fair_side
            prob,maturity_bucket,drift_scale=_ncaaf_v13_promoted_empirical_probability(preview,_thr,curg,side_orientation=orient)
            # Games 1-2 are a separate handicapping regime. If the regularized
            # situational brain earned historical OOS authority, it becomes the
            # primary probability source for those rows; otherwise the heavily
            # market-anchored empirical V13 probability remains in force.
            _esm=preview.get("early_situational_model")
            _escols=list(preview.get("early_situational_feature_cols") or [])
            if bool(preview.get("early_situational_gate_pass",False)) and _esm is not None and _escols:
                _esf=_ncaaf_v13_runtime_early_situational_features(out,Xstate).reindex(columns=_escols)
                try:
                    _esp=np.asarray(_esm.predict_proba(_esf)[:,1],dtype=float)
                    _early=np.isin(maturity_bucket,["FIRST_TWO_GAMES"]) & np.isfinite(_esp)
                    early_sit_prob[_early]=np.clip(_esp[_early],0.01,0.99)
                    prob[_early]=early_sit_prob[_early]
                    early_sit_active[_early]=True
                except Exception as _ese:
                    logging.warning("V13 early situational scoring unavailable: %s",_ese)
        elif isinstance(cover,dict) and cover.get("model") is not None:
            cf=pd.DataFrame(index=out.index)
        ...
        # V13.2 exact production recipe:
        #   Outcome AutoFS is PRIMARY -> cross-fit-selected core calibration ->
        #   optional Fundamental correction -> Market family residual -> individual Pathi/BigAl rules -> post-stack calibration.
        # Older V13/HF3 artifacts retain the legacy fundamental-base bridge path.
        # .27: for the Market-Residual secondary expert, use the legacy promoted
        # probability when it exists; otherwise fill only missing rows from the
        ...
        if _arch_pre.startswith("independent_fair_primary"):
            ...
            _fill=(~np.isfinite(_pp))&np.isfinite(_direct)
            if _fill.any(): _pp[_fill]=_direct[_fill]
            prob=_pp
        ...
        if _is_v13227:
            ...
            prob,_overlay_details,_overlay_gate=_apply_v13_specialist_overlays_runtime(
                ...
                core_calibrated_prob=own_anchor
            )
            out["V13_Base_Cover_Prob"]=own_anchor.astype("float32")
            ...
            out["V13_AutoFS_Core_Weight"]=np.float32(0.0)
            out["V13_AutoFS_Core_Active"]=np.int8(0)
            out["V13_AutoFS_Core_Contribution"]=np.float32(0.0)
        elif _is_v131:
            core_calibrated,_core_cal_details=_apply_v131_core_calibration_runtime(
                core_prob,bundle.get("core_calibration") or {},maturity=maturity_bucket
            )
            ...
            out["V13_Fundamental_Overlay_Regime"]=maturity_bucket
        ...
        for _fam in ("Market","Pathi","BigAl"):
            ...
            out[f"V13_{_fam}_Overlay_Weight"]=np.asarray(_dd.get("weight",np.zeros(n)),dtype="float32")
        ...
        out["V13_Own_Fair_Margin"]=np.asarray(own_fair_side,dtype="float32")
        out["V13_Own_Cover_Prob"]=np.asarray(own_prob,dtype="float32")
        out["V13_Own_Push_Prob"]=np.asarray(own_push,dtype="float32")
        out["V13_Market_Margin"]=np.asarray(offered_margin,dtype="float32")
        out["V13_Market_BreakEven_Prob"]=np.asarray(be,dtype="float32")
        _novig=np.asarray(_v13227_runtime_novig_spread_prob(out,be),dtype=float)
        out["V13_Market_NoVig_Prob"]=np.asarray(_novig,dtype="float32")
        out["V13_Point_Edge"]=np.asarray(own_fair_side-offered_margin,dtype="float32")
        _own_be_edge=np.asarray(own_prob-be,dtype=float)
        out["V13_Raw_Probability_Edge"]=np.asarray(_own_be_edge,dtype="float32")  # backward alias
        out["V13_Own_BreakEven_Edge"]=np.asarray(_own_be_edge,dtype="float32")
        out["V13_Market_Fair_Probability_Edge"]=np.asarray(own_prob-_novig,dtype="float32")
        out["V13_MarketResidual_Prob"]=np.asarray(fundamental_prob,dtype="float32")
        ...
        if _is_v13227:
            ...
            status=np.where(eligible,"V13_2_28_OWN_FAIR_ACTIVE","V13_2_28_OWN_FAIR_UNAVAILABLE")
        elif _is_v131:
            _early=np.isin(maturity_bucket,["FIRST_TWO_GAMES"])
        ...
        out["V13_Maturity_Beta"]=beta_arr.astype("float32")
        out["V13_Early_Situational_Prob"]=early_sit_prob.astype("float32")
        out["V13_Early_Situational_Active"]=early_sit_active.astype("int8")
        return out
    except Exception as e:
        logging.warning("V13 promoted NCAAF scoring unavailable: %s",e,exc_info=True)
        return out


def apply_ncaaf_statistical_brain_feature(df: pd.DataFrame, sb, market: str):
    ...
    try:
        ...
        # Spread anchor must come from a spread field.  A totals First_Line_Value
        # is a total, not a home spread, and H2H First_Line_Value is not a spread.
        open_line=None
        _spread_candidates = ("First_Line_Value","Opening_Spread","Open_Value","Opening_Line") if m=="spreads" else ("Opening_Spread",)
        for c in _spread_candidates:
            if c in out.columns:
                s=pd.to_numeric(out[c],errors="coerce")
                if s.notna().any(): open_line=s; break
        if open_line is None: open_line=pd.Series(np.nan,index=out.index,dtype=float)
        home_open_sp=np.where(is_home,open_line,np.where(is_away,-open_line,np.nan))
        market_margin=-np.asarray(home_open_sp,dtype=float)
        open_total=None
        for c in ("Opening_Total","First_Line_Value","Open_Value","Opening_Line"):
            if c in out.columns and m=="totals":
                s=pd.to_numeric(out[c],errors="coerce");
                if s.notna().any(): open_total=s; break
        if open_total is None:
            # totals are game-level; use explicit total aliases only when present
            for c in ("Opening_Total","TOT_Open","Open_Total"):
                if c in out.columns:
        ...
        if m=="spreads":
            ...
            resid=np.asarray(sb.get("residual_margin",[]),dtype=float)
            # reverse residual orientation for away side
            rawp=np.full(len(out),np.nan)
            ih=np.where(is_home.to_numpy())[0]; ia=np.where(is_away.to_numpy())[0]
            if ih.size: rawp[ih]=_ncaaf_stat_empirical_prob_gt(-(side_margin[ih]+current_value[ih]),resid)
            if ia.size: rawp[ia]=_ncaaf_stat_empirical_prob_gt(-(side_margin[ia]+current_value[ia]),-resid)
            exp_team=np.where(is_home,(exp_total+exp_margin)/2,np.where(is_away,(exp_total-exp_margin)/2,np.nan))
            exp_opp=exp_total-exp_team
        elif m=="totals":
            resid=np.asarray(sb.get("residual_total",[]),dtype=float); p_over=_ncaaf_stat_empirical_prob_gt(current_value-exp_total,resid)
            is_under=outcome.eq("under"); rawp=np.where(is_under,1-p_over,p_over); exp_team=(exp_total+exp_margin)/2; exp_opp=(exp_total-exp_margin)/2
        else:
        ...
        out.loc[eligible,"NCAAF_Stat_Expected_Team_Points"]=np.asarray(exp_team,dtype="float32")[eligible]
        out.loc[eligible,"NCAAF_Stat_Expected_Opp_Points"]=np.asarray(exp_opp,dtype="float32")[eligible]
        out.loc[eligible,"NCAAF_Stat_Prob"]=final[eligible].astype("float32")
        out.loc[eligible,"NCAAF_Stat_Edge"]=edge[eligible].astype("float32")
        out.loc[eligible,"NCAAF_Stat_Active"]=np.int8(1)
        out.loc[eligible,"NCAAF_Stat_Trust"]=eff[eligible].astype("float32")
        out.loc[eligible,"NCAAF_Stat_Uncertainty"]=(1-np.clip(eff[eligible]*sim[eligible],0,1)).astype("float32")
        return out
    except Exception as e:
        print(f"[NCAAF-STAT] scoring disabled: {e}")
        return out
...
def _hist_core_num_first(df: pd.DataFrame, *names, default=np.nan) -> pd.Series:
    for name in names:
        if name in df.columns:
...
def _hist_core_amer_prob(odds: pd.Series) -> pd.Series:
...
def _historical_core_runtime_baseline(df: pd.DataFrame, market: str) -> pd.Series:
    m = _sys_norm_market(market)
    if m in ("spreads", "totals"):
        return pd.Series(0.5, index=df.index, dtype="float64")
    fair = _hist_core_num_first(df, "Opening_ML_Fair_Prob")
    if fair.notna().any():
        return fair.clip(0.01, 0.99)
    odds = _hist_core_num_first(
        df, "Opening_ML_Odds", "First_Odds", "First_Odds_Price", "Open_Odds", "Open_Odds_Price", "Old_Odds"
    )
    raw = _hist_core_amer_prob(odds)
    group_cols = []
    if "Merge_Key_Short" in df.columns:
    ...
    if group_cols and raw.notna().any():
        keys = [df[c].astype(str) for c in group_cols]
        den = raw.groupby(keys, dropna=False).transform("sum")
        cnt = raw.notna().groupby(keys, dropna=False).transform("sum")
        devig = raw / den.where(den.gt(0))
        raw = raw.where(cnt.lt(2), devig)
    return raw.clip(0.01, 0.99).fillna(0.5)


def _historical_core_runtime_frame(df: pd.DataFrame, market: str) -> pd.DataFrame:
    m = _sys_norm_market(market)
    x = pd.DataFrame(index=df.index)
    week = _hist_core_num_first(df, "BigAl_Context_Week_Number", "Week_Number", "Week")
    is_conf = _hist_core_num_first(df, "BigAl_Context_Is_Conference_Game", "Is_Conference_Game")
    is_home = _hist_core_num_first(df, "BigAl_Context_Is_Home", "Is_Home")
    team_game = _hist_core_num_first(df, "BigAl_Context_Team_Game_Number", "Team_Game_Number")
    revenge = _hist_core_num_first(df, "Revenge_Flag_Current")
    ...
    x["HC_WinPct_Prior"] = _hist_core_num_first(df, "WinPct_Prior_System")
    x["HC_ATS_WinPct_Prior"] = _hist_core_num_first(df, "ATS_WinPct_Prior_System")
    x["HC_Opp_WinPct_Prior"] = _hist_core_num_first(df, "Opp_WinPct_Prior_System")
    x["HC_Prev_Points_For"] = _hist_core_num_first(df, "Prev_Points_For")
    x["HC_Prev_Points_Against"] = _hist_core_num_first(df, "Prev_Points_Against")
    x["HC_Avg_Points_For_Prior"] = _hist_core_num_first(df, "Avg_Points_For_Prior")
    x["HC_Avg_Points_Against_Prior"] = _hist_core_num_first(df, "Avg_Points_Against_Prior")
    x["HC_Revenge"] = revenge
    x["HC_Revenge_Known"] = revenge.notna().astype("float64")
    x["HC_Days_Since_Last_Game"] = _hist_core_num_first(df, "Days_Since_Last_Game_System")
    x["HC_Opp_Days_Since_Last_Game"] = _hist_core_num_first(df, "Opp_Days_Since_Last_Game_System")
    x["HC_Market_Baseline_Prob"] = _historical_core_runtime_baseline(df, m)
    if m == "spreads":
        op = _hist_core_num_first(df, "Opening_Spread", "First_Line_Value", "Open_Value", "Opening_Line")
        x["HC_Open_Line"] = op
        ...
        x["HC_Open_Line"] = _hist_core_num_first(df, "Opening_Total", "First_Line_Value", "Open_Value", "Opening_Line")
    else:
        odds = _hist_core_num_first(
            df, "Opening_ML_Odds", "First_Odds", "First_Odds_Price", "Open_Odds", "Open_Odds_Price", "Old_Odds"
        )
        x["HC_Open_Line"] = odds
        x["HC_Open_Implied_Prob"] = _hist_core_amer_prob(odds)
    return x.replace([np.inf, -np.inf], np.nan)


def _hist_core_logit(p):
    p = np.clip(np.asarray(p, dtype=float), 1e-5, 1 - 1e-5)
    return np.log(p / (1.0 - p))


def _hist_core_apply_calibrator(cal, raw_p):
    p = np.clip(np.asarray(raw_p, dtype=float), 0.01, 0.99)
    if cal is None:
        return p
    return np.asarray(cal.predict_proba(_hist_core_logit(p).reshape(-1, 1))[:, 1], dtype=float)


def _hist_core_raw_from_model(model, X, baseline):
...
def _hist_core_apply_system_memory(out: pd.DataFrame, hb: dict) -> pd.DataFrame:
    ...
    for fam in ("BigAl", "Pathi"):
        ...
        for name, stt in stats.items():
            ...
            nume += on*t*p; den += on*t; sample += on*n
        active = den > 0; post = np.full(len(out), 0.5, dtype=float); post[active] = nume[active] / den[active]
        out[f"{fam}_Historical_Posterior_Prob"] = post.astype("float32")
        out[f"{fam}_Historical_Trust"] = np.clip(den, 0.0, 1.0).astype("float32")
        out[f"{fam}_Historical_Sample"] = sample.astype("float32")
    return out


def apply_historical_core_runtime_feature(df: pd.DataFrame, bundle, market: str) -> pd.DataFrame:
    out = df.copy()
    defaults = {
        HISTORICAL_CORE_FEATURE_NAME: 0.5, HISTORICAL_CORE_RAW_NAME: 0.5, HISTORICAL_CORE_EDGE_NAME: 0.0,
        HISTORICAL_CORE_BASELINE_NAME: 0.5, HISTORICAL_CORE_ACTIVE_NAME: 0, HISTORICAL_CORE_TRUST_NAME: 0.0,
        HISTORICAL_CORE_BASE_TRUST_NAME: 0.0, HISTORICAL_CORE_DRIFT_NAME: 1.0, HISTORICAL_CORE_RECENCY_NAME: 1.0,
        HISTORICAL_CORE_AGREEMENT_NAME: 1.0, HISTORICAL_CORE_HORIZON_COUNT_NAME: 0, HISTORICAL_CORE_UNCERTAINTY_NAME: 1.0,
    }
    for c, v in defaults.items(): out[c] = v
    hb = bundle.get("historical_core_expert") if isinstance(bundle, dict) else None
    if not isinstance(hb, dict) or out.empty:
        return out
    try:
        m = _sys_norm_market(market)
        ...
        if not experts and hb.get("model") is not None:
            ...
            X = _historical_core_runtime_frame(out, m).reindex(columns=hb.get("feature_cols") or [])
            raw = np.asarray(hb["model"].predict_proba(X))[:, 1]
            eff = np.full(len(out), float(np.clip(hb.get("trust", 0.0), 0.0, 1.0)))
            sim = np.ones(len(out)); rec = np.ones(len(out)); agree = np.ones(len(out))
        else:
            probs=[]; weights=[]; sims=[]; recs=[]
            game_t = pd.to_datetime(out.get("Game_Start"), errors="coerce", utc=True)
            for ex in experts:
                X = _historical_core_runtime_frame(out, m).reindex(columns=ex.get("feature_cols") or [])
                rp = _hist_core_raw_from_model(ex["model"], X, baseline)
                ...
                t=float(np.clip(ex.get("trust",0.0),0.0,1.0)); w=t*s*r
                probs.append(p); weights.append(w); sims.append(s); recs.append(r)
            if not probs:
                raw=baseline.copy(); eff=np.zeros(len(out)); sim=np.ones(len(out)); rec=np.ones(len(out)); agree=np.zeros(len(out))
            else:
                P=np.vstack(probs); W=np.vstack(weights); S=np.vstack(sims); R=np.vstack(recs); den=W.sum(axis=0)
                raw=np.where(den>0,(P*W).sum(axis=0)/np.maximum(den,1e-12),baseline)
                eff=np.where(den>0,(W*W).sum(axis=0)/np.maximum(den,1e-12),0.0)
                agree=np.clip(1.0-np.nanstd(P,axis=0)/0.10,0.0,1.0) if P.shape[0]>=2 else np.ones(P.shape[1])
                eff=np.clip(eff*agree,0.0,1.0)
...
def override_corrected_line_move_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Override the existing line-movement fields with the canonical market-aware
    semantics used by training. No duplicate versioned movement columns are created.

    Spreads: support for a selected team means its spread number moved lower
             (-3 -> -4 favorite, +4 -> +3 dog).
    H2H:     support means the selected team's implied probability increased.
    ...
    spread = m.eq("spreads") & val.notna() & open_val.notna()
    toward.loc[spread] = val.loc[spread] < open_val.loc[spread]
    away.loc[spread] = val.loc[spread] > open_val.loc[spread]

    h2h = m.eq("h2h") & p_now.notna() & p_open.notna()
    toward.loc[h2h] = p_now.loc[h2h] > p_open.loc[h2h]
    away.loc[h2h] = p_now.loc[h2h] < p_open.loc[h2h]

    totals = m.eq("totals") & val.notna() & open_val.notna()
    over = totals & outn.eq("over")
    under = totals & outn.eq("under")
    toward.loc[over] = val.loc[over] > open_val.loc[over]
    away.loc[over] = val.loc[over] < open_val.loc[over]
    toward.loc[under] = val.loc[under] < open_val.loc[under]
    away.loc[under] = val.loc[under] > open_val.loc[under]

    out["Line_Moved_Toward_Team"] = toward.astype("int8")
    out["Line_Moved_Away_From_Team"] = away.astype("int8")
    out["Direction_Aligned"] = np.where(toward, 1, np.where(away, 0, -1)).astype("int8")

    abs_move = (val - open_val).abs()
    out["Abs_Line_Move_From_Opening"] = abs_move
    pct = pd.Series(
        np.where(totals & open_val.abs().gt(0), abs_move / open_val.abs(), np.nan),
        index=out.index,
        dtype="float64",
    )
    out["Pct_Line_Move_From_Opening"] = pct
    out["Pct_Line_Move_Bin"] = pd.cut(
        pct,
        bins=[-np.inf, 0.0025, 0.005, 0.01, 0.02, np.inf],
        labels=['<0.25%', '0.5%', '1%', '2%', '2%+'],
    )
    ...
    abs_z = abs_move.groupby([sport_norm, m], dropna=False).transform(_z).clip(-5, 5)
    pct_z = pct.groupby(sport_norm, dropna=False).transform(_z).clip(-5, 5)
    out["Abs_Line_Move_Z"] = abs_z
    out["Pct_Line_Move_Z"] = pct_z

    # Keep current odds movement features coherent too.
    out["Implied_Prob"] = p_now
    out["First_Imp_Prob"] = p_open
    out["Implied_Prob_Shift"] = p_now - p_open
    imp_z = out["Implied_Prob_Shift"].groupby([sport_norm, m], dropna=False).transform(_z).clip(-5, 5)
    out["Implied_Prob_Shift_Z"] = imp_z

    disabled = pd.to_numeric(out["Disable_Line_Move_Features"], errors="coerce").fillna(0).astype(int)
    out["Potential_Overmove_Flag"] = (
        m.eq("spreads") & toward & abs_z.ge(2) & disabled.eq(0)
    ).astype("int8")
    out["Potential_Overmove_Total_Pct_Flag"] = (
        m.eq("totals") & toward & pct_z.ge(2) & disabled.eq(0)
    ).astype("int8")
    out["Potential_Odds_Overmove_Flag"] = imp_z.ge(2).astype("int8")

    # Paired/de-vigged H2H fair probabilities on the physical game + bookmaker.
    # This is additive (non-versioned) and never changes row order/index.
    out["H2H_Fair_Prob"] = np.nan
    out["Opening_H2H_Fair_Prob"] = np.nan
    if "Merge_Key_Short" in out.columns:
        tmp_cols = [c for c in [
            "Merge_Key_Short", "Bookmaker", "Market", "Outcome_Norm", "Outcome",
            "Odds_Price", "Open_Odds", "First_Odds"
        ] if c in out.columns]
        tmp = out[tmp_cols].copy()
        tmp["__market"] = tmp.get("Market", "").astype(str).map(_sys_norm_market)
        tmp = tmp[tmp["__market"].eq("h2h")].copy()
        if not tmp.empty:
            tmp["__out"] = tmp.get("Outcome_Norm", tmp.get("Outcome", "")).astype(str).str.lower().str.strip()
            tmp["__p"] = _sys_amer_prob(pd.to_numeric(tmp.get("Odds_Price"), errors="coerce"))
            _oo = tmp.get("Open_Odds", tmp.get("First_Odds"))
            tmp["__p0"] = _sys_amer_prob(pd.to_numeric(_oo, errors="coerce"))
            gcols = ["Merge_Key_Short"] + (["Bookmaker"] if "Bookmaker" in tmp.columns else [])
            den = tmp.groupby(gcols, dropna=False)["__p"].transform("sum")
            den0 = tmp.groupby(gcols, dropna=False)["__p0"].transform("sum")
            tmp["__fair"] = tmp["__p"] / den.replace(0, np.nan)
            tmp["__fair0"] = tmp["__p0"] / den0.replace(0, np.nan)
            keycols = gcols + ["__out"]
            mp = tmp[keycols + ["__fair", "__fair0"]].drop_duplicates(keycols, keep="last")
            fair_map = {tuple(r[k] for k in keycols): (r["__fair"], r["__fair0"]) for _, r in mp.iterrows()}
            row_out = outn
            for i in out.index[h2h]:
                key = [out.at[i, "Merge_Key_Short"]]
                if "Bookmaker" in out.columns:
                    key.append(out.at[i, "Bookmaker"])
                key.append(row_out.at[i])
                vals = fair_map.get(tuple(key))
                if vals is not None:
                    out.at[i, "H2H_Fair_Prob"] = vals[0]
                    out.at[i, "Opening_H2H_Fair_Prob"] = vals[1]

    # Selected-row spread role vs H2H fair role where both are present.
    spread_role = np.where(val < 0, 1, np.where(val > 0, 0, np.nan))
    ml_role = np.where(
        pd.to_numeric(out["H2H_Fair_Prob"], errors="coerce") > 0.5, 1,
        np.where(pd.to_numeric(out["H2H_Fair_Prob"], errors="coerce") < 0.5, 0, np.nan)
    )
    out["Spread_vs_H2H_Role_Aligned"] = np.where(
        m.eq("spreads") & pd.notna(spread_role) & pd.notna(ml_role),
        (spread_role == ml_role).astype(float),
        np.nan,
    )
...
# ============================================================================
# End backend Pathi + Big Al adapters
# ============================================================================
...
def detect_sharp_moves(
    ...
    # Physical-game key used by the deterministic system layer.  Existing
    # Game_Key remains line/market/outcome specific for backward compatibility.
    df = _ensure_merge_key_short_for_systems(df)
...
def load_model_from_gcs(
    ...
    for m in market_candidates:
        ...
        return {
            ...
            "ncaaf_statistical_brain": payload.get("ncaaf_statistical_brain"),
            "ncaaf_v13_value_architecture": payload.get("ncaaf_v13_value_architecture"),
...
def write_to_bigquery(df, table='sharp_data.sharp_scores_full', force_replace=False):
    ...
    if table in allowed_cols:
        _sys_optional = _pathi_bigal_optional_cols(df)
        _sys_supported = _ensure_optional_bq_columns(df, table, _sys_optional)
        allowed_cols[table] += [c for c in _sys_supported if c not in allowed_cols[table]]
...
def fetch_scores_and_backtest(sport_key, df_moves=None, days_back=3, api_key=API_KEY, trained_models=None):
    ...
    def process_chunk_logic(df_chunk):
        ...
        # === Canonical market-aware line movement / alignment
        df_chunk = override_corrected_line_move_features(df_chunk)
    ...
    # Preserve any deterministic system/state features captured on the pregame rows.
    score_cols += [c for c in _pathi_bigal_optional_cols(df) if c not in score_cols]
    ...
    # System columns are additive and intentionally excluded from the legacy dedup fingerprint.
    # They are nevertheless retained for sharp_scores_full persistence/backtesting.
